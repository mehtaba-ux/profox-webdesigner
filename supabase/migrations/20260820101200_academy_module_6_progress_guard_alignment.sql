-- Module 6 alignment for the central Academy progress protection trigger.
-- Preserve existing protections while allowing the dedicated Loom RPC to clear/rewrite its own review fields during secure submit/resubmit.
...
BEGIN
  ...
  IF TG_OP='INSERT' THEN
    ...
    END IF;
    IF NEW.score IS NOT NULL AND v_quiz_rpc<>'1' AND v_product_rpc<>'1' AND v_niche_rpc<>'1' AND v_lead_research_rpc<>'1' AND v_loom_rpc<>'1' THEN
  ...
  ELSE
    ...
    IF (NEW.reviewed_by IS DISTINCT FROM OLD.reviewed_by OR NEW.reviewed_at IS DISTINCT FROM OLD.reviewed_at OR NEW.review_status IS DISTINCT FROM OLD.review_status OR NEW.feedback IS DISTINCT FROM OLD.feedback)
      ...
      RAISE EXCEPTION 'Training review fields are Admin-only.';
    END IF;
    IF NEW.score IS DISTINCT FROM OLD.score AND v_quiz_rpc<>'1' AND v_product_rpc<>'1' AND v_niche_rpc<>'1' AND v_lead_research_rpc<>'1' AND v_loom_rpc<>'1' THEN
      RAISE EXCEPTION 'Training scores must be recorded through the secure quiz/review workflow.';
    END IF;
