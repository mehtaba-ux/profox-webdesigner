-- Module 6 must be reviewed through the dedicated 100-point Loom rubric, never the legacy generic review path.

CREATE OR REPLACE FUNCTION public.admin_review_training_progress(p_progress_id uuid, p_status text, p_feedback text DEFAULT ''::text, p_score integer DEFAULT NULL::integer)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF v_module.slug IN ('mock-call-test','crm-training','final-certification') AND v_submission_count=0 THEN RAISE EXCEPTION 'A trainee submission is required before this module can be reviewed.'; END IF;
  IF p_status='Passed' AND v_module.slug='mock-call-test' THEN
    IF COALESCE(v_submission->>'type','')<>'mock_sales_call' OR COALESCE(v_submission->>'mode','') NOT IN ('live','recording') THEN RAISE EXCEPTION 'A valid live or recorded mock sales call submission is required.'; END IF;
    IF COALESCE(v_submission->>'mode','')='recording' AND COALESCE(trim(v_submission->>'videoUrl'),'')='' THEN RAISE EXCEPTION 'Recorded mock sales calls require a recording URL.'; END IF;
  END IF;
  IF p_status='Passed' AND v_module.slug='crm-training' AND COALESCE(v_submission->>'type','')<>'crm_practical' THEN RAISE EXCEPTION 'A CRM practical submission is required before Admin Pass.'; END IF;
