-- Module 12 — secure learner config, sequential lesson progress and server-scored certification.

create or replace function public.get_product_presentation_training_config(p_module_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_state public.product_presentation_training_state%rowtype;
...
begin
  ...
  select * into v_module from public.training_modules where id=p_module_id and slug='presentation-skills' and active=true;
  if not found then raise exception 'Product Presentation module not found.'; end if;
  ...
  if found then
    select * into v_state from public.product_presentation_training_state where progress_id=v_progress.id;
    ...
    where user_id=v_user and module_id=p_module_id and progress_id=v_progress.id
      and submission_data->>'type'='product_presentation_assessment_v1'
  ...
  return jsonb_build_object(
    'framework','HEARD → RECOMMEND → SHOW → PROVE → CONFIRM → ADVANCE',
    'principle','Do not show everything ProFox can do. Show exactly what this buyer needs to understand.',
    ...
    'operatingStandard',jsonb_build_object(
      'priorityLimit','1–3 buyer priorities',
      'proofRule','Verified and relevant only',
      'interactionRule','Short bursts + buyer dialogue',
      'commercialRule','Best fit, never highest price by default'
    )
  );
end;
$$;

create or replace function public.complete_product_presentation_lesson(p_progress_id uuid,p_lesson_id uuid)
returns jsonb
language plpgsql
security definer
set search_path=public,pg_temp
...
begin
  ...
  if not exists(select 1 from public.training_modules where id=v_progress.module_id and slug='presentation-skills' and active=true)
     or not public.can_access_sales_academy_module(v_progress.module_id) then
    raise exception 'Product Presentation training access denied.';
  end if;
  if v_progress.status in ('Passed','Completed') then return jsonb_build_object('complete',true,'certified',true); end if;

  insert into public.product_presentation_training_state(progress_id,user_id,module_id)
  values(v_progress.id,auth.uid(),v_progress.module_id)
  on conflict(progress_id) do nothing;
  select * into v_state from public.product_presentation_training_state where progress_id=v_progress.id for update;

  select count(*)::int into v_count from public.training_lessons where module_id=v_progress.module_id and active=true;
  if v_count=0 then raise exception 'Product Presentation lessons are missing.'; end if;

  select id into v_next
  from public.training_lessons
  where module_id=v_progress.module_id and active=true
  order by sort_order,id
  offset v_state.lessons_completed limit 1;
  ...
  if v_next<>p_lesson_id then raise exception 'Complete Product Presentation lessons in order.'; end if;

  v_done:=least(v_state.lessons_completed+1,v_count);
  v_percent:=least(70,round((v_done::numeric/greatest(v_count,1))*70)::int);
  update public.product_presentation_training_state set lessons_completed=v_done,updated_at=now() where progress_id=v_progress.id;
...
create or replace function public.submit_product_presentation_assessment(
  p_progress_id uuid,
  p_answers integer[],
...
begin
  ...
  select * into v_module from public.training_modules where id=v_progress.module_id and slug='presentation-skills' and active=true;
  if not found or not public.can_access_sales_academy_module(v_progress.module_id) then
    raise exception 'Product Presentation training access denied.';
  end if;
  if v_module.requires_admin_review then raise exception 'Module 12 is server-scored and must not require Admin review.'; end if;
  if v_progress.status in ('Passed','Completed') then raise exception 'Module 12 is already certified.'; end if;
  ...
  if v_missing_prior>0 then raise exception 'Complete all previous required Academy modules before submitting Module 12.'; end if;
  ...
  if v_lesson_count=0 or coalesce(v_state.lessons_completed,0)<v_lesson_count then
    raise exception 'Complete all Module 12 lessons before the Product Presentation assessment.';
  end if;
  ...
  if v_question_count=0 then raise exception 'No active Module 12 assessment questions are configured.'; end if;
  ...
  if v_confirmed_ack_count<>v_required_ack_count then
    raise exception 'Confirm every required Product Presentation acknowledgement before submitting.';
  end if;
  ...
  perform set_config('profox.training_product_presentation_rpc','1',true);
  ...
  values(auth.uid(),v_module.id,v_progress.id,jsonb_build_object(
    'type','product_presentation_assessment_v1',
    'answers',to_jsonb(p_answers),
    'acknowledgementIds',to_jsonb(coalesce(p_acknowledgement_ids,array[]::uuid[])),
    'score',v_score,
    'passed',v_passed,
    'criticalMisses',v_critical_misses,
    'feedback',v_feedback,
    'submittedAt',v_event_time
  ...
  perform set_config('profox.training_product_presentation_rpc','',true);
...
revoke all on function public.get_product_presentation_training_config(uuid) from public,anon;
revoke all on function public.complete_product_presentation_lesson(uuid,uuid) from public,anon;
revoke all on function public.submit_product_presentation_assessment(uuid,integer[],uuid[]) from public,anon;
grant execute on function public.get_product_presentation_training_config(uuid) to authenticated;
grant execute on function public.complete_product_presentation_lesson(uuid,uuid) to authenticated;
grant execute on function public.submit_product_presentation_assessment(uuid,integer[],uuid[]) to authenticated;
