-- UI/UX application form v2
-- Keeps hard security/identity requirements in the RPC and additionally enforces
-- Admin-configured visible + required fields and configured option lists.
...
begin
  ...
  -- Security/identity requirements remain non-configurable.
  ...
  -- Admin-editable v2 policy. If a legacy role has no fieldConfig, the hard
  -- requirements above remain active and this loop is intentionally a no-op.
  if jsonb_typeof(coalesce(v_job.role_details->'applicationForm'->'fieldConfig','{}'::jsonb))='object' then
    for v_field_key, v_field_cfg in
      select key, value from jsonb_each(coalesce(v_job.role_details->'applicationForm'->'fieldConfig','{}'::jsonb))
    loop
      if v_field_key='cv' then
        continue;
      end if;
      if coalesce((v_field_cfg->>'visible')::boolean,true) and coalesce((v_field_cfg->>'required')::boolean,false) then
        v_field_value:=coalesce(p_application->v_field_key, p_application->'answers'->v_field_key);
        v_field_label:=coalesce(nullif(trim(v_field_cfg->>'label'),''),v_field_key);
        if v_field_value is null
           or v_field_value='null'::jsonb
           or (jsonb_typeof(v_field_value)='string' and trim(v_field_value#>>'{}')='')
           or (jsonb_typeof(v_field_value)='array' and jsonb_array_length(v_field_value)=0)
           or (jsonb_typeof(v_field_value)='boolean' and v_field_value='false'::jsonb) then
          return jsonb_build_object('success',false,'field',v_field_key,'error',v_field_label||' is required.');
        end if;
      else
        v_field_value:=coalesce(p_application->v_field_key, p_application->'answers'->v_field_key);
      end if;
      ...
      if v_field_value is not null
         and jsonb_typeof(v_field_cfg->'options')='array'
         and jsonb_array_length(v_field_cfg->'options')>0 then
        if jsonb_typeof(v_field_value)='string' and not exists(
          select 1 from jsonb_array_elements_text(v_field_cfg->'options') option_row(value)
          where option_row.value=(v_field_value#>>'{}')
        ) then
          return jsonb_build_object('success',false,'field',v_field_key,'error','Select a valid option.');
        elsif jsonb_typeof(v_field_value)='array' and exists(
          select 1 from jsonb_array_elements_text(v_field_value) selected_row(value)
          where not exists(
            select 1 from jsonb_array_elements_text(v_field_cfg->'options') option_row(value)
            where option_row.value=selected_row.value
          )
        ) then
          return jsonb_build_object('success',false,'field',v_field_key,'error','One or more selected options are not valid.');
        end if;
      end if;
    end loop;
  end if;
  ...
  insert into public.applicants(
    ...
    v_reference,'uiux_role_v'||coalesce(nullif(v_job.role_details->'applicationForm'->>'version','')::integer,1),trim(coalesce(p_application->>'currentRole','')),v_hours,trim(coalesce(p_application->>'preferredWorkWindow','')),v_start,
