...
-- from evaluator-side recruitment assessments. Raw access tokens are generated
-- only at notification delivery time and are never persisted in the outbox.
...
create table if not exists public.recruitment_task_access_events (
  id bigint generated always as identity primary key,
  task_instance_id uuid not null references public.recruitment_task_instances(id) on delete cascade,
  action text not null check (action in ('open','save','submit')),
  occurred_at timestamptz not null default now()
);
create index if not exists recruitment_task_access_events_window_idx
  on public.recruitment_task_access_events(task_instance_id,action,occurred_at desc);
...
alter table public.recruitment_task_access_events enable row level security;

revoke all on public.recruitment_task_templates from public,anon,authenticated;
revoke all on public.recruitment_task_instances from public,anon,authenticated;
revoke all on public.recruitment_task_submissions from public,anon,authenticated;
revoke all on public.recruitment_task_access_events from public,anon,authenticated;
...
create or replace function public.check_recruitment_task_rate_limit(
  p_task_id uuid,
  p_action text,
...
as $$
begin
  if p_action not in ('open','save','submit') or p_max_per_hour < 1 then
    raise exception 'Invalid recruitment task rate-limit policy.';
  end if;
  delete from public.recruitment_task_access_events
  where task_instance_id=p_task_id and occurred_at < now()-interval '24 hours';
  if (select count(*) from public.recruitment_task_access_events
      where task_instance_id=p_task_id and action=p_action and occurred_at>now()-interval '1 hour') >= p_max_per_hour then
    raise exception 'Too many task requests. Please wait and try again.';
  end if;
  insert into public.recruitment_task_access_events(task_instance_id,action) values(p_task_id,p_action);
end;
$$;
revoke all on function public.check_recruitment_task_rate_limit(uuid,text,integer) from public,anon,authenticated;
...
begin
  ...
  if found and v_existing.status in ('Issued','Viewed','In Progress','Submitted','Under Review') then
    return jsonb_build_object(
      'instanceId',v_existing.id,'attemptNo',v_existing.attempt_no,'status',v_existing.status,'alreadyExists',true,
      'dueAt',v_existing.due_at,'estimatedMinutes',(v_existing.template_snapshot->>'estimatedMinutes')::integer,
      'requiredItems',(v_existing.template_snapshot->>'requiredItems')::integer,
      'targetMarket',v_existing.template_snapshot->>'targetMarket','targetNiche',v_existing.template_snapshot->>'targetNiche',
  ...
  v_snapshot:=jsonb_build_object(
    'taskKey',v_template.task_key,'title',v_template.title,'description',v_template.description,
    'targetMarket',v_template.target_market,'targetNiche',v_template.target_niche,
    'requiredItems',v_template.required_items,'deadlineHours',v_template.deadline_hours,
    'estimatedMinutes',v_template.estimated_minutes,'maxAttempts',v_template.max_attempts,
    'instructions',v_template.instructions,'version',v_template.version
  );
  ...
  insert into public.recruitment_task_instances(
    ...
    p_applicant_id,v_template.id,p_stage,v_attempt,'Issued',null,v_snapshot,coalesce(p_retry_feedback,''),v_due
  ...
  values(
    ...
    'recruitment_task_instances',v_instance.id,
  ...
  return jsonb_build_object(
    'instanceId',v_instance.id,'attemptNo',v_attempt,'status',v_instance.status,'alreadyExists',false,
    'dueAt',v_due,'estimatedMinutes',v_template.estimated_minutes,'requiredItems',v_template.required_items,
    'targetMarket',v_template.target_market,'targetNiche',v_template.target_niche,'retryFeedback',coalesce(p_retry_feedback,'')
  );
end;
$$;
...
begin
  ...
  return jsonb_build_object(
    'taskUrl','https://www.profoxwebdesigner.com/recruitment/task/'||v_token,
    ...
    'taskFeedback',v_task.retry_feedback
  );
end;
$$;
...
begin
  if new.stage is distinct from old.stage then
    ...
    if v_issue is not null then
      perform set_config(
        'profox.recruitment_task_email_context',
        jsonb_build_object('taskInstanceId',v_issue->>'instanceId')::text,
        true
      );
    end if;
  end if;
...
begin
  if new.closed_at is not null or new.stage is distinct from old.stage then
    ...
    where applicant_id=new.id
      ...
      and (new.closed_at is not null or stage<>new.stage);
  end if;
  return new;
end;
$$;
drop trigger if exists trg_close_inactive_recruitment_tasks on public.applicants;
create trigger trg_close_inactive_recruitment_tasks
after update of stage,closed_at on public.applicants
for each row execute function public.close_inactive_recruitment_tasks();
...
begin
  ...
  v_can_edit:=v_task.status in ('Issued','Viewed','In Progress')
    and v_task.due_at>=v_now and v_app.closed_at is null and v_app.stage=v_task.stage;

  return jsonb_build_object(
    'title',v_task.template_snapshot->>'title','description',v_task.template_snapshot->>'description',
    ...
    'attemptNo',v_task.attempt_no,'maxAttempts',(v_task.template_snapshot->>'maxAttempts')::integer,
    'candidateName',v_app.full_name,'applicationReference',v_app.application_reference,
    'status',v_task.status,'issuedAt',v_task.issued_at,'dueAt',v_task.due_at,'submittedAt',v_task.submitted_at,
    'retryFeedback',v_task.retry_feedback,'expired',v_task.due_at<v_now,'canEdit',v_can_edit,
    'answers',case when v_task.submitted_at is null then coalesce(v_submission.draft_data,'{"leads":[]}'::jsonb)
                   else coalesce(v_submission.final_data,v_submission.draft_data) end
  );
end;
$$;
...
begin
  ...
  where token_hash is not null and token_hash=public.recruitment_task_token_hash(p_token) for update;
  if not found then raise exception 'This task link is invalid or no longer active.'; end if;
  ...
  on conflict(task_instance_id) do update set
    draft_data=excluded.draft_data,save_count=public.recruitment_task_submissions.save_count+1,
...
begin
  ...
  on conflict(task_instance_id) do update set
    ...
    save_count=public.recruitment_task_submissions.save_count+1,last_saved_at=now(),submitted_at=now(),updated_at=now();

  update public.recruitment_task_instances
  set status='Submitted',submitted_at=now(),last_saved_at=now(),updated_at=now()
  where id=v_task.id returning * into v_task;
  ...
  loop
    perform public.enqueue_in_app_notification(
      v_admin.id,'Recruitment','Lead Research Test ready for review',
      v_app.full_name||' submitted Lead Research Test attempt '||v_task.attempt_no||'.',
      '/admin/app/recruitment?tab=recruitment','recruitment-task-review:'||v_task.id::text||':'||v_admin.id::text
    );
  end loop;
...
begin
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'estimatedMinutes',(t.template_snapshot->>'estimatedMinutes')::integer,
    'maxAttempts',(t.template_snapshot->>'maxAttempts')::integer,
    ...
    'finalData',case when t.submitted_at is not null then s.final_data else null end
...
begin
  ...
  if v_task.status='Submitted' then
    update public.recruitment_task_instances set status='Under Review',updated_at=now() where id=v_task.id returning * into v_task;
  end if;
...
begin
  ...
  perform public.enqueue_notification(
    ...
    v_template_key,v_app.email,null,v_payload,now()
  );
  insert into public.applicant_events(applicant_id,category,event_type,title,detail,actor_type,actor_user_id,source_table,source_id,metadata)
  values(v_task.applicant_id,'Recruitment','Task Link Resent','Practical task link resent','Admin invalidated the previous task link and requested a new secure link.','Admin',auth.uid(),'recruitment_task_instances',v_task.id,jsonb_build_object('attemptNo',v_task.attempt_no));
...
begin
  ...
  select * into v_app from public.applicants where id=v_task.applicant_id;
  update public.recruitment_task_instances
  set due_at=greatest(due_at,now())+make_interval(hours=>p_hours),token_hash=null,updated_at=now()
  where id=v_task.id returning * into v_task;
  v_payload:=public.recruitment_notification_payload(v_app)||jsonb_build_object(
    'taskInstanceId',v_task.id,'taskExtensionHours',p_hours
  );
  perform public.enqueue_notification(
    'recruitment-task:'||v_task.id::text||':extension:'||extract(epoch from clock_timestamp())::bigint,
    'recruitment_task_deadline_extended',v_app.email,null,v_payload,now()
  );
  insert into public.applicant_events(applicant_id,category,event_type,title,detail,actor_type,actor_user_id,source_table,source_id,metadata)
  values(v_task.applicant_id,'Recruitment','Task Deadline Extended','Practical task deadline extended','Admin extended the deadline by '||p_hours||' hours and invalidated the previous link.','Admin',auth.uid(),'recruitment_task_instances',v_task.id,jsonb_build_object('attemptNo',v_task.attempt_no,'dueAt',v_task.due_at,'extensionHours',p_hours));
...
create or replace function public.admin_get_recruitment_task_template(
  p_system_role text default 'sales',
  p_stage text default 'Lead Research Test'
)
returns jsonb
language plpgsql
security definer
...
begin
  if not public.is_admin() then raise exception 'Administrator access required.'; end if;
  select * into v_t from public.recruitment_task_templates
  where active=true and system_role=p_system_role and stage=p_stage
  order by version desc limit 1;
  if not found then return null; end if;
  return jsonb_build_object(
    'id',v_t.id,'taskKey',v_t.task_key,'systemRole',v_t.system_role,'stage',v_t.stage,
    'title',v_t.title,'description',v_t.description,'targetMarket',v_t.target_market,'targetNiche',v_t.target_niche,
    'requiredItems',v_t.required_items,'deadlineHours',v_t.deadline_hours,'estimatedMinutes',v_t.estimated_minutes,
    'maxAttempts',v_t.max_attempts,'instructions',v_t.instructions,'version',v_t.version,'updatedAt',v_t.updated_at
  );
end;
$$;
...
begin
  ...
  if exists(select 1 from public.recruitment_task_templates where active=true and system_role=v_role and stage=new.stage) then
    select * into v_task from public.recruitment_task_instances
    where applicant_id=new.applicant_id and stage=new.stage
    order by attempt_no desc limit 1;
    ...
    if new.attempt_no<>v_task.attempt_no then
      raise exception 'Assessment attempt must match the latest submitted practical-task attempt.';
    end if;
...
declare
  ...
  v_feedback text;
begin
  if not exists(select 1 from public.recruitment_task_templates where active=true and system_role=public.career_job_system_role(new.job_id) and stage=new.stage) then
    return new;
  end if;
  select * into v_task from public.recruitment_task_instances
