...
begin
  ...
  if v_q.opportunity_id is null then
    ...
    return jsonb_build_object(
      ...
      'scopeConditionCoverage','[]'::jsonb,'promiseCoverage','[]'::jsonb,'validationConflicts','[]'::jsonb,
      ...
      'quotationDimensions',jsonb_build_array(
        jsonb_build_object('key','PROMISE_COVERAGE','status',case when v_historical then 'WARNING' else 'BLOCKED' end,'coverageState',case when v_historical then 'LEGACY_NOT_CAPTURED' else 'NOT_EVALUATED' end),
  ...
  loop
    ...
    if not v_historical and v_effective<>'COVERED' then
      v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object(
        'code',case when v_effective='STALE' then 'PROMISE_COVERAGE_STALE' when v_effective='CONFLICT' then 'PROMISE_COVERAGE_CONFLICT' else 'PROMISE_COVERAGE_REQUIRED' end,
        'message','Active '||replace(v_promise.promise_type,'_',' ')||' Promise is not currently covered by a reviewed customer-visible quotation target.',
    ...
    v_promise_rows:=v_promise_rows||jsonb_build_array(jsonb_build_object(
      'promiseId',v_promise.id,'promiseType',v_promise.promise_type,'promiseText',v_promise.promise_text,
      'promisedBy',v_promise.promised_by,'promisedAt',v_promise.promised_at,'sourceType',v_promise.source_type,'sourceSummary',v_promise.source_summary,
      ...
      'targetType',case when v_has_cov then v_cov.target_type else null end,'quotationFieldKey',case when v_has_cov then v_cov.quotation_field_key else null end,'quotationItemId',case when v_has_cov then v_cov.quotation_item_id else null end,
      'targetExcerpt',case when v_has_cov then v_evidence->>'excerpt' else null end,'reviewedBy',case when v_has_cov then v_cov.reviewed_by else null end,'reviewedAt',case when v_has_cov then v_cov.reviewed_at else null end,'coverageNote',case when v_has_cov then v_cov.coverage_note else null end,
      'eligibleTargets',v_eligible,'futureSendBlockerStatus',case when v_integrity_blocked or v_effective<>'COVERED' then 'BLOCKED' else 'PASS' end
    ));
  end loop;

  if v_promise_count=0 then v_promise_status:='PASS';
  elsif jsonb_array_length(coalesce(v_scope_assessment#>'{promises,blockers}','[]'::jsonb))>0 or (not v_historical and v_promise_covered<v_promise_count) then v_promise_status:='BLOCKED';
  ...
  return jsonb_build_object(
    ...
    'scopeConditionCoverage',v_condition_rows,'promiseCoverage',v_promise_rows,
    ...
    'quotationDimensions',jsonb_build_array(
      jsonb_build_object('key','PROMISE_COVERAGE','status',v_promise_status,'coverageState',case when v_historical then 'LEGACY_NOT_CAPTURED' when v_promise_count=0 then 'NO_MATERIAL_PROMISES_REGISTERED' when v_promise_status='PASS' then 'COVERED' else 'NEEDS_RECONCILIATION' end,'activeCount',v_promise_count,'coveredCount',v_promise_covered),
...
begin
  ...
  if v_lead_id is not null then
    select coalesce(jsonb_agg(jsonb_build_object(
      'id',c.id,'versionSourceId',c.supersedes_condition_id,'conditionType',c.condition_type,'title',c.title,'exactWording',c.condition_text,
      'sourceType',c.source_type,'sourceRequirementId',c.source_requirement_id,'sourceValidationId',c.source_validation_id,'sourceMeetingId',c.source_meeting_id,'sourceSummary',c.source_summary,
      'validationAlignmentStatus',c.validation_alignment_status,'activatedAt',c.activated_at
    ...
    select coalesce(jsonb_agg(jsonb_build_object(
      'id',p.id,'versionSourceId',p.supersedes_promise_id,'promiseType',p.promise_type,'exactWording',p.promise_text,
      'promisedBy',p.promised_by,'promisedAt',p.promised_at,'sourceType',p.source_type,'sourceSummary',p.source_summary,
      'linkedRequirementId',p.linked_requirement_id,'linkedValidationId',p.linked_validation_id,'validationAlignmentStatus',p.validation_alignment_status
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    'quotationItemId',qi.id,'salesProductId',qi.sales_product_id,'productCode',qi.product_code_snapshot,'productName',qi.product_name_snapshot,
    'descriptionSnapshot',qi.description_snapshot,'configurationSnapshot',qi.configuration_snapshot,'clientExpectationsSnapshot',qi.client_expectations_snapshot,
    'optionalForClient',qi.optional_for_client,'lineType',qi.line_type,'itemType',qi.item_type
  ...
  return jsonb_build_object(
    ...
    'policyKey',v_assessment->>'policyKey','policyVersion',v_assessment->'policyVersion',
    'quotationId',v_q.id,'quotationRevision',v_q.revision_number,'opportunityId',v_q.opportunity_id,'leadId',v_assessment->'leadId',
    'evaluationTimestamp',v_assessment->'evaluatedAt',
    'proposalReadiness',jsonb_build_object('status',v_assessment#>>'{proposalReadiness,status}','policyKey',v_assessment#>>'{proposalReadiness,policyKey}','policyVersion',v_assessment#>'{proposalReadiness,policyVersion}'),
    'packageFit',jsonb_build_object('status',v_assessment#>>'{packageFit,status}','policyKey',v_assessment#>>'{packageFit,policyKey}','policyVersion',v_assessment#>'{packageFit,policyVersion}','recommendedProduct',v_assessment#>'{packageFit,recommendedProduct}'),
    'quotedProducts',v_products,
    'activeScopeConditions',v_scope,
    'activePromises',v_promises,
    'coverageMappings',jsonb_build_object('scopeConditions',v_assessment->'scopeConditionCoverage','promises',v_assessment->'promiseCoverage'),
    'approvedConstraints',v_assessment->'approvedConstraints',
    'finalReconciliationStatus',v_assessment->>'status',
    'quotationDimensions',v_assessment->'quotationDimensions',
...
grant execute on function public.crm_build_quotation_sales_scope_snapshot(uuid) to authenticated;
