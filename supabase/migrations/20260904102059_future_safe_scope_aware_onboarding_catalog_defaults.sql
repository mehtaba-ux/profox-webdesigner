  ...
  case v_code
    ...
    when 'PF-CARE-PRIORITY' then v_keys:=array['maintenancePriorities','analyticsAccess','seoAssets','strategyPriorities','technicalAccess'];
      ...
      if v_type='package' and v_category like '%web%' then
        ...
        if v_hint like '%email%' or v_hint like '%newsletter%' then v_keys:=array['emailAutomationRequirements','technicalAccess'];
...
begin
  ...
  if p_product ? 'onboarding_requirements' then
    ...
    select onboarding_requirements into v_current_requirements from public.sales_products where id=(p_product->>'id')::uuid;
    v_requirements:=coalesce(v_current_requirements,public.client_onboarding_default_requirements(v_code,v_name,v_category,v_product_type,v_scope));
  else
