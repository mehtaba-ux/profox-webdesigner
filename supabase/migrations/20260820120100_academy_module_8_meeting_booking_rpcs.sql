-- Sales Academy Module 8 — secure learner workflow.

CREATE OR REPLACE FUNCTION public.get_meeting_booking_training_config(p_module_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
DECLARE
  ...
  v_state public.meeting_booking_training_state%ROWTYPE;
  v_lesson_count integer:=0;
  v_questions jsonb:='[]'::jsonb;
  v_acknowledgements jsonb:='[]'::jsonb;
  v_latest jsonb;
  v_meeting_settings jsonb:='{}'::jsonb;
  v_booking_settings jsonb:='{}'::jsonb;
BEGIN
  IF v_user IS NULL THEN RAISE EXCEPTION 'Authentication required.'; END IF;

  SELECT * INTO v_module
  FROM public.training_modules
  WHERE id=p_module_id AND slug='meeting-booking' AND active=true;
  IF NOT FOUND THEN RAISE EXCEPTION 'Meeting Booking module not found.'; END IF;

  IF NOT public.can_access_sales_academy_module(p_module_id) THEN
    RAISE EXCEPTION 'Training module access denied.';
  END IF;
  ...
  IF FOUND THEN
    SELECT * INTO v_state
    FROM public.meeting_booking_training_state
    ...
    WHERE user_id=v_user
      ...
      AND progress_id=v_progress.id
      AND submission_data->>'type'='meeting_booking_assessment_v1'
    ORDER BY created_at DESC,id DESC
    LIMIT 1;
  END IF;

  SELECT count(*)::int INTO v_lesson_count
  ...
  SELECT config_value INTO v_booking_settings
  FROM public.system_configuration
  WHERE config_key='public_booking_settings';

  RETURN jsonb_build_object(
    'framework','READY → REASON → VALUE → CHOICE → CONFIRM',
    'passingScore',COALESCE(v_module.passing_score,80),
    ...
    'meetingSettings',COALESCE(v_meeting_settings,'{}'::jsonb),
    'publicBookingSettings',COALESCE(v_booking_settings,'{}'::jsonb)
  );
END;
$function$;

CREATE OR REPLACE FUNCTION public.complete_meeting_booking_lesson(p_progress_id uuid,p_lesson_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public','pg_temp'
...
BEGIN
  ...
  IF NOT EXISTS(
    ...
    WHERE id=v_progress.module_id AND slug='meeting-booking' AND active=true
  ) OR NOT public.can_access_sales_academy_module(v_progress.module_id) THEN
    RAISE EXCEPTION 'Meeting Booking access denied.';
  END IF;
  ...
  INSERT INTO public.meeting_booking_training_state(progress_id,user_id,module_id)
  ...
  IF v_count=0 THEN RAISE EXCEPTION 'Meeting Booking lessons are missing.'; END IF;
  ...
  END IF;
  IF v_next<>p_lesson_id THEN
    RAISE EXCEPTION 'Complete Meeting Booking lessons in order.';
  END IF;

  v_done:=LEAST(v_state.lessons_completed+1,v_count);
  v_percent:=LEAST(70,round((v_done::numeric/GREATEST(v_count,1))*70)::int);

  UPDATE public.meeting_booking_training_state
  SET lessons_completed=v_done,updated_at=now()
  WHERE progress_id=v_progress.id;

  UPDATE public.user_training_progress
  SET status='In Progress',
      progress_percent=GREATEST(COALESCE(progress_percent,0),v_percent),
      updated_at=now()
  ...
  RETURN jsonb_build_object(
    ...
    'lessonCount',v_count,
    'complete',v_done>=v_count,
    'progressPercent',v_percent
  );
END;
$function$;

CREATE OR REPLACE FUNCTION public.submit_meeting_booking_assessment(
  p_progress_id uuid,
  p_answers integer[],
...
BEGIN
  ...
  IF v_module.requires_admin_review THEN
    RAISE EXCEPTION 'Module 8 must remain a one-time server-scored certification without recurring Admin approval.';
  END IF;
  IF v_progress.status IN ('Passed','Completed') THEN
    RAISE EXCEPTION 'Module 8 is already certified. Normal future meeting booking does not require per-meeting Admin approval.';
  END IF;
  ...
  IF v_missing_prior>0 THEN
    RAISE EXCEPTION 'Complete all previous required Academy modules before submitting Module 8.';
  END IF;
  ...
  IF v_lesson_count=0 OR COALESCE(v_state.lessons_completed,0)<v_lesson_count THEN
    RAISE EXCEPTION 'Complete all Module 8 lessons before the practical assessment.';
  END IF;

  SELECT count(*)::int INTO v_question_count
  FROM public.training_assessment_questions
  WHERE module_id=v_module.id AND active=true;
  IF v_question_count=0 THEN RAISE EXCEPTION 'No active Module 8 assessment questions are configured.'; END IF;
  ...
  FROM public.training_acknowledgements
  ...
  IF v_confirmed_ack_count<>v_required_ack_count THEN
    RAISE EXCEPTION 'Confirm every required Meeting Booking acknowledgement before submitting.';
  END IF;
  ...
  PERFORM set_config('profox.training_meeting_booking_rpc','1',true);

  INSERT INTO public.training_assignments(user_id,module_id,progress_id,submission_data,created_at,updated_at)
  VALUES(
    auth.uid(),v_module.id,v_progress.id,
    jsonb_build_object(
      'type','meeting_booking_assessment_v1',
  ...
  PERFORM set_config('profox.training_meeting_booking_rpc','',true);
...
REVOKE ALL ON FUNCTION public.get_meeting_booking_training_config(uuid) FROM PUBLIC,anon;
REVOKE ALL ON FUNCTION public.complete_meeting_booking_lesson(uuid,uuid) FROM PUBLIC,anon;
REVOKE ALL ON FUNCTION public.submit_meeting_booking_assessment(uuid,integer[],uuid[]) FROM PUBLIC,anon;

GRANT EXECUTE ON FUNCTION public.get_meeting_booking_training_config(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.complete_meeting_booking_lesson(uuid,uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.submit_meeting_booking_assessment(uuid,integer[],uuid[]) TO authenticated;
