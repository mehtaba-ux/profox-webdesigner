...
begin
  ...
  if not public.can_manage_content_applicant(v_assessment.applicant_id) and not public.is_admin() then
    raise exception 'Recruitment management access required.';
  end if;

  select * into v_app
  from public.applicants
  where id=v_assessment.applicant_id
  for update;
  if not found then raise exception 'Candidate not found.'; end if;

  if coalesce(btrim(v_app.refusal_reason),'')<>'' then
    raise exception 'Closed candidates cannot have assessments edited.';
  end if;

  if v_app.stage<>v_assessment.stage then
    raise exception 'Historical assessments cannot be edited after the candidate leaves that stage.';
  end if;

  if exists(
    select 1
    from public.recruitment_assessments newer
    where newer.applicant_id=v_assessment.applicant_id
      and newer.stage=v_assessment.stage
      and newer.attempt_no>v_assessment.attempt_no
  ) then
    raise exception 'Only the latest assessment attempt for the current stage can be edited.';
  end if;

  select * into v_policy
  from public.recruitment_stage_policies
  where job_id=public.recruitment_job_for_applicant(v_app.id)
    and stage=v_assessment.stage
    and active=true;
  ...
  if p_status='Passed' and p_score<v_passing_score then
    raise exception 'Passed assessment score must meet the saved passing score of %.',v_passing_score;
  end if;
  if p_status='Passed' and jsonb_array_length(coalesce(p_critical_failures,'[]'::jsonb))>0 then
    raise exception 'An assessment with critical failures cannot be passed.';
  end if;
  if p_status='Passed' and v_policy.interview_required
     and not public.recruitment_interview_requirement_satisfied(v_assessment.applicant_id,v_assessment.stage) then
    raise exception 'Complete or administratively skip the required % interview before marking this assessment Passed.',v_assessment.stage;
  end if;

  v_previous_status:=v_assessment.status;
  v_previous_score:=v_assessment.score;

  update public.recruitment_assessments
  set status=p_status,
      score=p_score,
      rubric_scores=coalesce(p_rubric_scores,'{}'::jsonb),
      critical_failures=coalesce(p_critical_failures,'[]'::jsonb),
      evidence=left(coalesce(p_evidence,''),10000),
      evidence_url=left(coalesce(p_evidence_url,''),2000),
      evaluator_notes=left(coalesce(p_notes,''),10000),
  ...
  perform public.log_applicant_event(
    v_assessment.applicant_id,
    'assessment',
    'assessment_updated',
    v_assessment.stage||' assessment updated',
    coalesce(nullif(btrim(p_notes),''),'Latest structured recruitment assessment updated.'),
    v_previous_status,
    p_status,
    ...
    v_assessment.id,
    jsonb_build_object(
      'stage',v_assessment.stage,
      'attemptNo',v_assessment.attempt_no,
...
begin
  ...
  v_assessment:=public.admin_update_recruitment_assessment(
    p_assessment_id,
    p_status,
    p_score,
    p_rubric_scores,
  ...
  if p_status='Passed' then
    v_progression:=public.admin_advance_applicant_stage(v_assessment_row.applicant_id,null);
  end if;
