-- Advertise Professional Email only when the current user passes the same CRM
-- lead-access and professional-mailbox eligibility checks used by the send RPC.
...
begin
  ...
  from public.system_configuration where config_key='whatsapp_business';
  ...
  v_can_email:=
    v_conv.crm_lead_id is not null
