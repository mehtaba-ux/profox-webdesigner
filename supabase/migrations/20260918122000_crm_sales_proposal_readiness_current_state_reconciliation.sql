-- Part 10B.6 forward-only current-state reconciliation: Proposal Readiness.
-- NEW migration superseding the required current state of both unresolved Part 8 historical files.
-- Evaluator 1 and evaluator 2 are NOT restored; evaluator 3 is the sole canonical authority.
...
declare
  ...
  v_bad integer;
begin
  if to_regclass('public.crm_opportunities') is null
     ...
     or to_regclass('public.crm_sales_promises') is null then
    raise exception '[P10B6_READINESS_PRECONDITION] required Part 6/7/9/current Sales domain tables are missing.';
  end if;

  if to_regprocedure('public.crm_get_package_fit_assessment(uuid,uuid)') is null
     or to_regprocedure('public.crm_get_sales_scope_commitment_assessment(uuid)') is null
     or to_regprocedure('public.is_admin()') is null
     or to_regprocedure('public.has_active_role(text[])') is null then
    raise exception '[P10B6_READINESS_PRECONDITION] required Package Fit, scope/promise, or access dependency is missing.';
  end if;
  ...
  if v_policy is not null
     and v_policy is distinct from $p10b6_gate_v3${"gates":{"PROPOSAL_READINESS":{"statusValues":["READY","WARNING","BLOCKED"],"coverageState":"FUTURE_DIMENSIONS_PENDING"},"REQUIREMENTS_CONFIRMED":{"statusValues":["PASS","WARNING","BLOCKED"]}},"policyKey":"crm_sales_gate_policy_v1","nextAction":{"genericSubjects":["tbd","follow up","follow-up","wait","waiting"],"requiredForOpenOpportunity":true},"policyVersion":1,"supportedGates":["REQUIREMENTS_CONFIRMED","PROPOSAL_READINESS"],"certaintyStates":["CLIENT_CONFIRMED","SELLER_OBSERVATION","SELLER_HYPOTHESIS","AWAITING_CLIENT","NEEDS_SPECIALIST_VALIDATION","NOT_APPLICABLE"],"validationTypes":["TECHNICAL","COMMERCIAL","TIMELINE","COMPLIANCE_RISK","SCOPE"],"evaluatorVersion":3,"packageFitBehavior":{"FIT":"PASS","MISMATCH":"BLOCKED","POSSIBLE_FIT":"WARNING","REVIEW_REQUIRED":"RESOLVE_REVIEWS"},"proposalDimensions":["BUSINESS_CONTEXT","PROBLEM","IMPACT","DESIRED_OUTCOME","AUDIENCE","SCOPE","PACKAGE_FIT","TECHNICAL_VALIDATION","COMMERCIAL_VALIDATION","TIMELINE_VALIDATION","COMPLIANCE_RISK_VALIDATION","DECISION_PROCESS","REQUIREMENTS_COMPLETENESS","MEETING_DISCOVERY_EVIDENCE","NEXT_ACTION","ASSUMPTIONS","EXCLUSIONS","DEPENDENCIES","SCOPE_CONDITIONS_REGISTER","PROMISE_REGISTER_INTEGRITY"],"requirementClasses":["CORE","RECOMMENDED","CONDITIONAL","COMPLEX"],"complexPackageCodes":["PF-WEB-SCALE","PF-CUSTOM"],"warningCoreCategories":["CONTENT","BRAND","SEO","TECHNICAL"],"blockingCoreCategories":["BUSINESS","PROBLEM","DESIRED_OUTCOME","AUDIENCE_CUSTOMER","PROJECT_SCOPE","COMMERCIAL","DECISION_BUYING_PROCESS","RISKS_DEPENDENCIES"],"futureProposalDimensions":["PROMISE_COVERAGE","FINAL_SCOPE_RECONCILIATION","QUOTATION_SNAPSHOT_COVERAGE"],"validationSeverityBehavior":{"RED":"BLOCKED","AMBER":"BLOCKED","GREEN":"WARNING"},"observationalRequirementKeys":["current_situation","existing_digital_state","domain_status","hosting_status","cms_platform"],"complexDecisionRequirementKeys":["economic_buyer","decision_criteria","decision_process","procurement_process","stakeholder_map","internal_champion"]}$p10b6_gate_v3$::jsonb
     and v_policy is distinct from $p10b6_gate_v2${"gates":{"PROPOSAL_READINESS":{"statusValues":["READY","WARNING","BLOCKED"],"coverageState":"FUTURE_DIMENSIONS_PENDING"},"REQUIREMENTS_CONFIRMED":{"statusValues":["PASS","WARNING","BLOCKED"]}},"policyKey":"crm_sales_gate_policy_v1","nextAction":{"genericSubjects":["tbd","follow up","follow-up","wait","waiting"],"requiredForOpenOpportunity":true},"policyVersion":1,"supportedGates":["REQUIREMENTS_CONFIRMED","PROPOSAL_READINESS"],"certaintyStates":["CLIENT_CONFIRMED","SELLER_OBSERVATION","SELLER_HYPOTHESIS","AWAITING_CLIENT","NEEDS_SPECIALIST_VALIDATION","NOT_APPLICABLE"],"validationTypes":["TECHNICAL","COMMERCIAL","TIMELINE","COMPLIANCE_RISK","SCOPE"],"evaluatorVersion":2,"packageFitBehavior":{"FIT":"PASS","MISMATCH":"BLOCKED","POSSIBLE_FIT":"WARNING","REVIEW_REQUIRED":"RESOLVE_REVIEWS"},"proposalDimensions":["BUSINESS_CONTEXT","PROBLEM","IMPACT","DESIRED_OUTCOME","AUDIENCE","SCOPE","PACKAGE_FIT","TECHNICAL_VALIDATION","COMMERCIAL_VALIDATION","TIMELINE_VALIDATION","COMPLIANCE_RISK_VALIDATION","DECISION_PROCESS","REQUIREMENTS_COMPLETENESS","MEETING_DISCOVERY_EVIDENCE","NEXT_ACTION","ASSUMPTIONS","EXCLUSIONS","DEPENDENCIES"],"requirementClasses":["CORE","RECOMMENDED","CONDITIONAL","COMPLEX"],"complexPackageCodes":["PF-WEB-SCALE","PF-CUSTOM"],"warningCoreCategories":["CONTENT","BRAND","SEO","TECHNICAL"],"blockingCoreCategories":["BUSINESS","PROBLEM","DESIRED_OUTCOME","AUDIENCE_CUSTOMER","PROJECT_SCOPE","COMMERCIAL","DECISION_BUYING_PROCESS","RISKS_DEPENDENCIES"],"futureProposalDimensions":["PROMISE_COVERAGE","FINAL_SCOPE_RECONCILIATION","QUOTATION_SNAPSHOT_COVERAGE"],"validationSeverityBehavior":{"RED":"BLOCKED","AMBER":"BLOCKED","GREEN":"WARNING"},"observationalRequirementKeys":["current_situation","existing_digital_state","domain_status","hosting_status","cms_platform"],"complexDecisionRequirementKeys":["economic_buyer","decision_criteria","decision_process","procurement_process","stakeholder_map","internal_champion"]}$p10b6_gate_v2$::jsonb then
    raise exception '[P10B6_READINESS_PRECONDITION] unknown or newer Sales gate policy state.';
  end if;
  ...
  if v_pipeline is null
     or jsonb_typeof(v_pipeline->'stages') is distinct from 'array' then
    raise exception '[P10B6_READINESS_PRECONDITION] crm_pipeline_settings is missing or malformed.';
  end if;

  select count(*) into v_stage_count
  from jsonb_array_elements(v_pipeline->'stages')
  where value->>'name'='Requirements Confirmed';
  select value into v_req_stage
  from jsonb_array_elements(v_pipeline->'stages')
  where value->>'name'='Requirements Confirmed'
  limit 1;

  if v_stage_count<>1
     or v_req_stage is null
     or coalesce((v_req_stage->>'active')::boolean,false)<>true
     or jsonb_typeof(coalesce(v_req_stage->'requiredFields','[]'::jsonb)) is distinct from 'array' then
    raise exception '[P10B6_READINESS_PRECONDITION] Requirements Confirmed stage is missing, duplicated, inactive, or malformed.';
  end if;

  select count(*) into v_bad
  from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='public' and p.proname='crm_get_sales_gate_assessment'
    and pg_get_function_identity_arguments(p.oid)<>'p_opportunity_id uuid, p_gate_key text';
  if v_bad<>0 then
    raise exception '[P10B6_READINESS_PRECONDITION] unexpected crm_get_sales_gate_assessment overload.';
  end if;

  select count(*) into v_bad
  from pg_proc p join pg_namespace n on n.oid=p.pronamespace
  where n.nspname='public' and p.proname='crm_transition_opportunity'
    and pg_get_function_identity_arguments(p.oid)<>'p_opportunity_id uuid, p_target_stage text';
  if v_bad<>0 then
    raise exception '[P10B6_READINESS_PRECONDITION] unexpected crm_transition_opportunity overload.';
  end if;
  ...
  perform set_config(
    'profox.part10b6.requirements_confirmed_count',
    (select count(*)::text from public.crm_opportunities where stage='Requirements Confirmed'),
    true
  );
end
$p10b6_pre$;
...
values ('crm_sales_gate_policy_v1',$p10b6_gate_v3${"gates":{"PROPOSAL_READINESS":{"statusValues":["READY","WARNING","BLOCKED"],"coverageState":"FUTURE_DIMENSIONS_PENDING"},"REQUIREMENTS_CONFIRMED":{"statusValues":["PASS","WARNING","BLOCKED"]}},"policyKey":"crm_sales_gate_policy_v1","nextAction":{"genericSubjects":["tbd","follow up","follow-up","wait","waiting"],"requiredForOpenOpportunity":true},"policyVersion":1,"supportedGates":["REQUIREMENTS_CONFIRMED","PROPOSAL_READINESS"],"certaintyStates":["CLIENT_CONFIRMED","SELLER_OBSERVATION","SELLER_HYPOTHESIS","AWAITING_CLIENT","NEEDS_SPECIALIST_VALIDATION","NOT_APPLICABLE"],"validationTypes":["TECHNICAL","COMMERCIAL","TIMELINE","COMPLIANCE_RISK","SCOPE"],"evaluatorVersion":3,"packageFitBehavior":{"FIT":"PASS","MISMATCH":"BLOCKED","POSSIBLE_FIT":"WARNING","REVIEW_REQUIRED":"RESOLVE_REVIEWS"},"proposalDimensions":["BUSINESS_CONTEXT","PROBLEM","IMPACT","DESIRED_OUTCOME","AUDIENCE","SCOPE","PACKAGE_FIT","TECHNICAL_VALIDATION","COMMERCIAL_VALIDATION","TIMELINE_VALIDATION","COMPLIANCE_RISK_VALIDATION","DECISION_PROCESS","REQUIREMENTS_COMPLETENESS","MEETING_DISCOVERY_EVIDENCE","NEXT_ACTION","ASSUMPTIONS","EXCLUSIONS","DEPENDENCIES","SCOPE_CONDITIONS_REGISTER","PROMISE_REGISTER_INTEGRITY"],"requirementClasses":["CORE","RECOMMENDED","CONDITIONAL","COMPLEX"],"complexPackageCodes":["PF-WEB-SCALE","PF-CUSTOM"],"warningCoreCategories":["CONTENT","BRAND","SEO","TECHNICAL"],"blockingCoreCategories":["BUSINESS","PROBLEM","DESIRED_OUTCOME","AUDIENCE_CUSTOMER","PROJECT_SCOPE","COMMERCIAL","DECISION_BUYING_PROCESS","RISKS_DEPENDENCIES"],"futureProposalDimensions":["PROMISE_COVERAGE","FINAL_SCOPE_RECONCILIATION","QUOTATION_SNAPSHOT_COVERAGE"],"validationSeverityBehavior":{"RED":"BLOCKED","AMBER":"BLOCKED","GREEN":"WARNING"},"observationalRequirementKeys":["current_situation","existing_digital_state","domain_status","hosting_status","cms_platform"],"complexDecisionRequirementKeys":["economic_buyer","decision_criteria","decision_process","procurement_process","stakeholder_map","internal_champion"]}$p10b6_gate_v3$::jsonb)
on conflict (config_key) do update
set config_value=excluded.config_value,
    updated_at=now()
where public.system_configuration.config_value is distinct from excluded.config_value;

-- Remove only the obsolete duplicate narrative stage requirement when present.
-- Preserve every unrelated pipeline property and avoid repeated version increments on canonical state.
update public.system_configuration sc
set config_value=jsonb_set(
      jsonb_set(
        sc.config_value,
        '{version}',
        to_jsonb(greatest(coalesce((sc.config_value->>'version')::integer,1),1)+1),
        true
      ),
      '{stages}',
      (
        select jsonb_agg(
          case
            when stage->>'name'='Requirements Confirmed'
            then jsonb_set(
              stage,
              '{requiredFields}',
              coalesce((
                select jsonb_agg(value)
                from jsonb_array_elements(coalesce(stage->'requiredFields','[]'::jsonb)) value
                where trim(both '"' from value::text)<>'requirementsSummary'
              ),'[]'::jsonb),
              true
            )
            else stage
...
where sc.config_key='crm_pipeline_settings'
  and exists (
    select 1
    from jsonb_array_elements(sc.config_value->'stages') stage,
         jsonb_array_elements_text(coalesce(stage->'requiredFields','[]'::jsonb)) field
    where stage->>'name'='Requirements Confirmed'
      and field='requirementsSummary'
  );

CREATE OR REPLACE FUNCTION public.crm_get_sales_gate_assessment(p_opportunity_id uuid, p_gate_key text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
begin
  ...
  if v_policy_version is distinct from 1 or v_evaluator_version is distinct from 3 then
  ...
  if jsonb_typeof(v_policy->'proposalDimensions') is distinct from 'array'
     or jsonb_array_length(v_policy->'proposalDimensions')<>20 then
  ...
  return jsonb_build_object(
    ...
    'scopeCommitment',case when v_gate_key='PROPOSAL_READINESS' then v_scope_commitment else null end,
...
revoke all on function public.crm_get_sales_gate_assessment(uuid,text)
  from public, anon, authenticated, service_role;
grant execute on function public.crm_get_sales_gate_assessment(uuid,text)
  to authenticated, service_role;

revoke all on function public.crm_transition_opportunity(uuid,text)
  from public, anon, authenticated, service_role;
grant execute on function public.crm_transition_opportunity(uuid,text)
  to authenticated, service_role;
...
begin
  ...
  if v_policy is distinct from $p10b6_gate_v3${"gates":{"PROPOSAL_READINESS":{"statusValues":["READY","WARNING","BLOCKED"],"coverageState":"FUTURE_DIMENSIONS_PENDING"},"REQUIREMENTS_CONFIRMED":{"statusValues":["PASS","WARNING","BLOCKED"]}},"policyKey":"crm_sales_gate_policy_v1","nextAction":{"genericSubjects":["tbd","follow up","follow-up","wait","waiting"],"requiredForOpenOpportunity":true},"policyVersion":1,"supportedGates":["REQUIREMENTS_CONFIRMED","PROPOSAL_READINESS"],"certaintyStates":["CLIENT_CONFIRMED","SELLER_OBSERVATION","SELLER_HYPOTHESIS","AWAITING_CLIENT","NEEDS_SPECIALIST_VALIDATION","NOT_APPLICABLE"],"validationTypes":["TECHNICAL","COMMERCIAL","TIMELINE","COMPLIANCE_RISK","SCOPE"],"evaluatorVersion":3,"packageFitBehavior":{"FIT":"PASS","MISMATCH":"BLOCKED","POSSIBLE_FIT":"WARNING","REVIEW_REQUIRED":"RESOLVE_REVIEWS"},"proposalDimensions":["BUSINESS_CONTEXT","PROBLEM","IMPACT","DESIRED_OUTCOME","AUDIENCE","SCOPE","PACKAGE_FIT","TECHNICAL_VALIDATION","COMMERCIAL_VALIDATION","TIMELINE_VALIDATION","COMPLIANCE_RISK_VALIDATION","DECISION_PROCESS","REQUIREMENTS_COMPLETENESS","MEETING_DISCOVERY_EVIDENCE","NEXT_ACTION","ASSUMPTIONS","EXCLUSIONS","DEPENDENCIES","SCOPE_CONDITIONS_REGISTER","PROMISE_REGISTER_INTEGRITY"],"requirementClasses":["CORE","RECOMMENDED","CONDITIONAL","COMPLEX"],"complexPackageCodes":["PF-WEB-SCALE","PF-CUSTOM"],"warningCoreCategories":["CONTENT","BRAND","SEO","TECHNICAL"],"blockingCoreCategories":["BUSINESS","PROBLEM","DESIRED_OUTCOME","AUDIENCE_CUSTOMER","PROJECT_SCOPE","COMMERCIAL","DECISION_BUYING_PROCESS","RISKS_DEPENDENCIES"],"futureProposalDimensions":["PROMISE_COVERAGE","FINAL_SCOPE_RECONCILIATION","QUOTATION_SNAPSHOT_COVERAGE"],"validationSeverityBehavior":{"RED":"BLOCKED","AMBER":"BLOCKED","GREEN":"WARNING"},"observationalRequirementKeys":["current_situation","existing_digital_state","domain_status","hosting_status","cms_platform"],"complexDecisionRequirementKeys":["economic_buyer","decision_criteria","decision_process","procurement_process","stakeholder_map","internal_champion"]}$p10b6_gate_v3$::jsonb
     or (v_policy->>'policyVersion')::integer<>1
     or (v_policy->>'evaluatorVersion')::integer<>3 then
    raise exception '[P10B6_READINESS_POLICY_V1_EVALUATOR_V3] canonical policy/evaluator version mismatch.';
  end if;

  if jsonb_array_length(v_policy->'proposalDimensions')<>20
     or jsonb_array_length(v_policy->'futureProposalDimensions')<>3
     or not (v_policy->'proposalDimensions' ?& array['SCOPE_CONDITIONS_REGISTER','PROMISE_REGISTER_INTEGRITY'])
     or not (v_policy->'futureProposalDimensions' ?& array['PROMISE_COVERAGE','FINAL_SCOPE_RECONCILIATION','QUOTATION_SNAPSHOT_COVERAGE']) then
    raise exception '[P10B6_READINESS_DIMENSIONS_20_PLUS_3] current/deferred dimension contract mismatch.';
  end if;

  if md5(pg_get_functiondef(to_regprocedure('public.crm_get_sales_gate_assessment(uuid,text)')))<>'1a1ecb061cdcee8e8115b845bd69d698'
     or not (
       select prosecdef
          and provolatile='s'
          and proconfig=array['search_path=""']::text[]
       from pg_proc
       where oid=to_regprocedure('public.crm_get_sales_gate_assessment(uuid,text)')
     ) then
    raise exception '[P10B6_READINESS_EVALUATOR_CURRENT] evaluator definition/security contract mismatch.';
  end if;

  v_def:=pg_get_functiondef(to_regprocedure('public.crm_get_sales_gate_assessment(uuid,text)'));
  if position('crm_get_sales_scope_commitment_assessment' in v_def)=0
     or position('SCOPE_CONDITIONS_REGISTER' in v_def)=0
     or position('PROMISE_REGISTER_INTEGRITY' in v_def)=0 then
    raise exception '[P10B6_READINESS_PART9_SCOPE_PROMISE] Part 9 scope/promise integration is missing.';
  end if;
  ...
  if v_stage_count<>1
     or exists (
       select 1
       from jsonb_array_elements_text(coalesce(v_req_stage->'requiredFields','[]'::jsonb)) f
       where f='requirementsSummary'
     ) then
    raise exception '[P10B6_READINESS_PIPELINE_INVARIANT] Requirements Confirmed stage invariant failed.';
  end if;

  if has_function_privilege('anon',to_regprocedure('public.crm_get_sales_gate_assessment(uuid,text)'),'EXECUTE')
     or not has_function_privilege('authenticated',to_regprocedure('public.crm_get_sales_gate_assessment(uuid,text)'),'EXECUTE')
     or not has_function_privilege('service_role',to_regprocedure('public.crm_get_sales_gate_assessment(uuid,text)'),'EXECUTE')
     or has_function_privilege('anon',to_regprocedure('public.crm_transition_opportunity(uuid,text)'),'EXECUTE')
     or not has_function_privilege('authenticated',to_regprocedure('public.crm_transition_opportunity(uuid,text)'),'EXECUTE')
     or not has_function_privilege('service_role',to_regprocedure('public.crm_transition_opportunity(uuid,text)'),'EXECUTE') then
    raise exception '[P10B6_READINESS_ACLS_CURRENT] readiness RPC ACL mismatch.';
  end if;

  if (select count(*) from public.crm_opportunities)
       <>current_setting('profox.part10b6.opportunity_count')::bigint
     or (select count(*) from public.crm_opportunities where stage='Requirements Confirmed')
       <>current_setting('profox.part10b6.requirements_confirmed_count')::bigint then
    raise exception '[P10B6_READINESS_BUSINESS_COUNTS_UNCHANGED] opportunity/stage counts changed.';
  end if;
