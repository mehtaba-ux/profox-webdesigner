-- Module 16 — secure learner config, sequential lessons and server-scored payment certification.

create or replace function public.get_payment_process_training_config(p_module_id uuid)
returns jsonb
language plpgsql
stable
security definer
...
declare
  ...
  v_state public.payment_process_training_state%rowtype;
...
begin
  ...
  select * into v_module from public.training_modules where id=p_module_id and slug='payment-process' and active=true;
  if not found then raise exception 'Payment Process module not found.'; end if;
  ...
  if found then
    select * into v_state from public.payment_process_training_state where progress_id=v_progress.id;
    ...
    where user_id=v_user and module_id=p_module_id and progress_id=v_progress.id
      and submission_data->>'type'='payment_process_assessment_v1'
  ...
  return jsonb_build_object(
    ...
    'principle','Request clearly. Verify independently. Advance only after confirmed payment.',
    ...
    'operatingStandard',jsonb_build_object(
      'salesAuthority','Sales may prepare, send and follow up on approved payment requests; Sales does not decide financial truth.',
      'verificationRule','Only the protected Admin verification workflow may set a payment to Verified.',
      'sourceOfTruth','Accepted quotation + live Admin-configured package payment schedule + current CRM payment record.',
      'activationRule','A fully verified qualifying Advance/Full Payment can trigger the protected Won/client handoff; partial payment cannot.'
    )
  );
end;
$$;

create or replace function public.complete_payment_process_lesson(p_progress_id uuid,p_lesson_id uuid)
returns jsonb
language plpgsql
security definer
set search_path=public,pg_temp
...
begin
  ...
  if not exists(select 1 from public.training_modules where id=v_progress.module_id and slug='payment-process' and active=true)
     or not public.can_access_sales_academy_module(v_progress.module_id) then
    raise exception 'Payment Process training access denied.';
  end if;
  if v_progress.status in ('Passed','Completed') then return jsonb_build_object('complete',true,'certified',true); end if;

  insert into public.payment_process_training_state(progress_id,user_id,module_id)
  values(v_progress.id,auth.uid(),v_progress.module_id)
  on conflict(progress_id) do nothing;
  select * into v_state from public.payment_process_training_state where progress_id=v_progress.id for update;

  select count(*)::int into v_count from public.training_lessons where module_id=v_progress.module_id and active=true;
  if v_count=0 then raise exception 'Payment Process lessons are missing.'; end if;
  ...
  if v_next<>p_lesson_id then raise exception 'Complete Module 16 lessons in order.'; end if;

  v_done:=least(v_state.lessons_completed+1,v_count);
  v_percent:=least(70,round((v_done::numeric/greatest(v_count,1))*70)::int);
  update public.payment_process_training_state set lessons_completed=v_done,updated_at=now() where progress_id=v_progress.id;
...
create or replace function public.submit_payment_process_assessment(
  p_progress_id uuid,
  p_answers integer[],
...
begin
  ...
  select * into v_module from public.training_modules where id=v_progress.module_id and slug='payment-process' and active=true;
  if not found or not public.can_access_sales_academy_module(v_progress.module_id) then
    raise exception 'Payment Process training access denied.';
  end if;
  if v_module.requires_admin_review then raise exception 'Module 16 is server-scored and must not require Admin review.'; end if;
  if v_progress.status in ('Passed','Completed') then raise exception 'Module 16 is already certified.'; end if;
  ...
  if v_missing_prior>0 then raise exception 'Complete all previous required Academy modules before submitting Module 16.'; end if;
  ...
  if v_lesson_count=0 or coalesce(v_state.lessons_completed,0)<v_lesson_count then
    raise exception 'Complete all Module 16 lessons before the Payment Process assessment.';
  end if;
  ...
  if v_question_count=0 then raise exception 'No active Module 16 assessment questions are configured.'; end if;
  ...
  if v_confirmed_ack_count<>v_required_ack_count then
    raise exception 'Confirm every required Payment Process acknowledgement before submitting.';
  end if;
  ...
  perform set_config('profox.training_payment_process_rpc','1',true);
  ...
  values(auth.uid(),v_module.id,v_progress.id,jsonb_build_object(
    'type','payment_process_assessment_v1',
  ...
  perform set_config('profox.training_payment_process_rpc','',true);
...
revoke all on function public.get_payment_process_training_config(uuid) from public,anon;
revoke all on function public.complete_payment_process_lesson(uuid,uuid) from public,anon;
revoke all on function public.submit_payment_process_assessment(uuid,integer[],uuid[]) from public,anon;
grant execute on function public.get_payment_process_training_config(uuid) to authenticated;
grant execute on function public.complete_payment_process_lesson(uuid,uuid) to authenticated;
grant execute on function public.submit_payment_process_assessment(uuid,integer[],uuid[]) to authenticated;
