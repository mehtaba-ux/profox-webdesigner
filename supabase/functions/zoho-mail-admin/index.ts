...
function html(title:string,message:string,success=false){
 const safeTitle=escapeHtml(title);const safeMessage=escapeHtml(message);const payload=scriptSafeJson({type:"profox-zoho-mail-oauth",success,message});
...
Deno.serve(async(req:Request)=>{
 ...
 try{
   ...
   if(!tokenResponse.ok||!tokenPayload?.access_token||!tokenPayload?.refresh_token){const detail=String(tokenPayload?.error_description||tokenPayload?.error||"Zoho did not issue the required refresh authorization.").slice(0,300);return html("Zoho connection could not be verified",`${detail} No mailbox provisioning was enabled.`);}
   ...
   if(!verifyResponse.ok){const detail=String(verifyPayload?.status?.description||verifyPayload?.data?.errorCode||`Zoho Mail organization verification failed (${verifyResponse.status}).`).slice(0,350);return html("Zoho Mail access was not verified",`${detail} Check the Organization ID, Mail plan/API entitlement and OAuth permissions. No mailbox provisioning was enabled.`);}
   const {error:storeError}=await service.rpc("service_store_verified_zoho_org_mail_connection",{p_admin_user_id:String(stateRow.admin_user_id),p_refresh_token:refreshToken,p_organization_id:org,p_data_center:dc,p_scopes:["ZohoMail.organization.accounts.ALL"]});
