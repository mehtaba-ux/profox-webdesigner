...
Deno.serve(async(req:Request)=>{
 ...
 async function failOrRetry(job:any,message:string,httpStatus:number|null=null,forceRetry=false){const retry=forceRetry&&Number(job.attempts)<maxAttempts;if(retry){const delay=retryDelay(Number(job.attempts),baseRetry);await finish(job,"retry",message,delay);await audit(job,"retry",httpStatus,`${message} · retry in ${delay}s`);}else{await finish(job,"failed",message);await service.rpc("service_mark_google_connection_state",{p_user_id:job.user_id,p_status:"error",p_error:message,p_success:false});await audit(job,"failed",httpStatus,message);}}
