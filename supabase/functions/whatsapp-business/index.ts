...
function safeDetail(payload: any, fallback: string) {
  return String(
    payload?.error?.message || payload?.error?.error_user_msg || payload?.message || fallback,
  ).slice(0, 500);
}
...
function extractMessageText(message: any) {
  ...
  if (["image", "video", "audio", "document", "sticker", "location", "contacts"].includes(type)) {
...
Deno.serve(async (req: Request) => {
  ...
  const graphApiVersion = /^v[0-9]+\.[0-9]+$/.test(String(credentials?.graphApiVersion || ""))
    ? String(credentials.graphApiVersion)
    : "v23.0";
  ...
  if (req.method === "GET") {
    ...
    if (!configured || mode !== "subscribe" || !challenge || !timingSafeEqual(suppliedVerifyToken, verifyToken)) {
  ...
  // Authenticated ProFox send path. The browser never receives provider credentials.
  if (authHeader.startsWith("Bearer ") && contentType.toLowerCase().includes("application/json")) {
    ...
    if (String(prepared?.status) === "provider_accepted") {
      return json({
        ...
        requestId,
        providerMessageId: String(prepared?.providerMessageId || ""),
        recipientPhone: String(prepared?.recipientPhone || ""),
      });
    }
    if (String(prepared?.status) === "failed") {
      return json({ error: "This WhatsApp request has already failed. Create a new message request." }, 409);
    }
    ...
    const { data: requestRow, error: requestError } = await service
      ...
      .maybeSingle();
    if (requestError || !requestRow || String(requestRow.status) !== "pending") {
      return json({ error: "WhatsApp send request could not be loaded safely." }, 500);
    }
    ...
    return json({
      ...
      requestId,
      providerMessageId: messageId,
      recipientPhone: String(requestRow.recipient_phone),
    });
  }

  // Public Meta webhook path. Raw-body HMAC verification happens before JSON parsing.
  ...
  for (const entry of Array.isArray(payload?.entry) ? payload.entry : []) {
    for (const change of Array.isArray(entry?.changes) ? entry.changes : []) {
      ...
      for (const messageItem of Array.isArray(value?.messages) ? value.messages : []) {
        ...
        const { data: resolution, error: resolutionError } = await service.rpc("service_resolve_client_whatsapp_conversation", {
          p_customer_phone: fromPhone,
        });
        ...
        const { error: upsertError } = await service.rpc("service_upsert_client_whatsapp_message", {
          ...
          p_sent_or_received_at: asIsoTimestamp(messageItem?.timestamp),
        });
