...
const PRODUCTION_ORIGIN = "https://www.profoxwebdesigner.com";
...
function cors(origin: string | null) {
  const allowed = origin && allowedOrigins.has(origin) ? origin : PRODUCTION_ORIGIN;
  return {
    "Access-Control-Allow-Origin": allowed,
    "Access-Control-Allow-Headers": "authorization, x-client-info, apikey, content-type, x-profox-cron-token, x-profox-academy-reissue",
...
function buildSetupUrl(systemRole: string, trainingTrack: string, tokenHash: string, verificationType: "invite" | "recovery") {
  const path = systemRole === "content_writer" ? "/team-onboarding/setup" : "/sales-onboarding/setup";
  const params = new URLSearchParams({
    track: trainingTrack,
    token_hash: tokenHash,
    type: verificationType,
  });
  return `${PRODUCTION_ORIGIN}${path}?${params.toString()}`;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const academyReissue = req.headers.get("x-profox-academy-reissue") === "1";
    ...
    if (!cronAuthorized) {
      ...
      const { data: profile, error: profileError } = await service.from("user_profiles").select("id,role,status").eq("id", caller.id).maybeSingle();
      if (profileError || !profile || profile.status !== "active") return json(headers, 403, { error: "Active staff access required" });
    ...
    const { data: job, error: jobError } = applicant.career_job_id
      ? await service.from("career_jobs").select("application_type,role_details").eq("id", applicant.career_job_id).maybeSingle()
    ...
    if (!new Set(["sales", "content_writer", "uiux_designer", "developer"]).has(systemRole)) return json(headers, 409, { error: "This role does not have a protected onboarding account workflow yet" });
    if (cronAuthorized && systemRole !== "content_writer" && !(systemRole === "sales" && academyReissue)) return json(headers, 403, { error: "Automated provisioning is not authorized for this role" });

    if (!cronAuthorized) {
      const callerRole = String(callerProfile?.role || "");
      if (systemRole === "content_writer") {
        if (!["admin", "editor", "site_manager"].includes(callerRole)) return json(headers, 403, { error: "Content recruitment management access required" });
      } else if (callerRole !== "admin") return json(headers, 403, { error: "Active Admin access required" });
    }

    if (String(applicant.refusal_reason || "").trim()) return json(headers, 409, { error: "Closed candidates cannot receive onboarding access" });
    ...
    const allowedStages = systemRole === "content_writer"
      ...
      : systemRole === "uiux_designer"
        ...
        : systemRole === "developer"
          ...
          : ["Selected", "Agreement Pending", "Sales Academy Training"];
    if (!allowedStages.includes(String(applicant.stage))) return json(headers, 409, { error: "Candidate is not in an account invitation stage" });
    if (systemRole !== "content_writer" && String(applicant.agreement_status || "").toLowerCase() !== "signed") return json(headers, 409, { error: "A verified candidate agreement is required before Academy access" });

    if (Number(applicant.onboarding_invite_count || 0) >= 20) return json(headers, 429, { error: "Invitation resend limit reached. Review the candidate account before trying again" });
    if (!academyReissue && applicant.onboarding_invite_last_sent_at) {
      const elapsed = Date.now() - new Date(applicant.onboarding_invite_last_sent_at).getTime();
      if (Number.isFinite(elapsed) && elapsed >= 0 && elapsed < 120_000) return json(headers, 429, { error: "An Academy access invitation was sent recently. Please wait before resending" });
    }
    ...
    if (!targetUserId) {
      const { data: profiles, error: lookupError } = await service.from("user_profiles").select("id,email,role,status").ilike("email", email).limit(1);
      if (lookupError) throw new Error(lookupError.message || "Candidate account lookup failed");
      if (profiles?.[0]?.id) targetUserId = String(profiles[0].id);
    }
    ...
    if (targetUserId) {
      ...
      const { data, error } = await service.auth.admin.generateLink({ type: "invite", email, options: { data: { full_name: String(applicant.full_name || "").trim() } } });
    ...
    if (!targetUserId || !tokenHash) {
      ...
      throw new Error("Secure account setup token could not be generated");
    }
    const accountInviteUrl = buildSetupUrl(systemRole, trainingTrack, tokenHash, inviteMode);

    let linked: any;
    try {
      const { error: profileUpsertError } = await service.from("user_profiles").upsert({ id: targetUserId, email, full_name: String(applicant.full_name || "").trim() }, { onConflict: "id", ignoreDuplicates: true });
      ...
      const { data, error: linkedError } = await service.rpc(rpcName, { p_applicant_id: applicantId, p_target_user_id: targetUserId, p_account_invite_url: accountInviteUrl });
    ...
    const academyStage = systemRole === "content_writer" ? "Content Academy" : systemRole === "uiux_designer" ? "Design Academy" : systemRole === "developer" ? "Developer Academy" : "Sales Academy Training";
