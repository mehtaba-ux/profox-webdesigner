...
export const OPPORTUNITY_STAGE_GUIDANCE: Record<OpportunityStage, SellerGuidanceEntry> = {
  ...
  'Negotiation / Decision Pending': make('pipeline.stage.Negotiation / Decision Pending', 'Negotiation / Decision Pending', 'Negotiation is not a parking stage. Record the customer’s actual decision state, keep one explicit opportunity-linked next action with an owner and due date, and route commercial exceptions through existing approval authority.', { sellerAction: 'Record only what the customer actually communicated. If you are waiting on the customer, still schedule the next check-in.', avoid: 'Do not infer a price or budget objection from silence, promise unapproved scope/discounts/dates, mark customer acceptance manually, or mark the opportunity Won.', sopReference: 'Part 11 Negotiation / Decision Pending' }),
...
export const SELLER_GUIDANCE: Record<string, SellerGuidanceEntry> = {
  ...
  'section.negotiation_next_action': make('section.negotiation_next_action', 'Decision & Next Action', 'Keep the opportunity’s current decision state and its canonical CRM next action visible together. Decision state belongs to the opportunity; the next action, owner and due date remain CRM Activities.', { sellerAction: 'Record the truthful decision state, then make sure the earliest Scheduled activity explicitly linked to this opportunity represents the next committed action.', avoid: 'Do not use generic notes as a substitute for decision state, and do not create duplicate reminder/task records.', sopReference: 'Part 11 Decision & Next Action' }),
  'field.negotiation_decision_status': make('field.negotiation_decision_status', 'Decision status', 'Choose the controlled status that matches what is actually known about the customer decision.', { certaintyNote: 'Silence means awaiting response, not automatically Price, Budget, Authority or another objection.', sopReference: 'Part 11 Decision State' }),
  'field.negotiation_waiting_on': make('field.negotiation_waiting_on', 'Waiting on', 'Record who currently controls the dependency. Waiting on the customer still requires a scheduled re-check/follow-up date.', { avoid: 'Waiting externally is not permission to leave the opportunity without a next action.', sopReference: 'Part 11 Waiting State' }),
  'field.negotiation_objection': make('field.negotiation_objection', 'Primary objection', 'Record an objection category only when the customer actually raised one or it is otherwise directly known.', { avoid: 'Do not infer an objection from non-response or Seller speculation.', sopReference: 'Part 11 Objection State' }),
  'action.schedule_negotiation_next': make('action.schedule_negotiation_next', 'Schedule next action', 'Creates the current next action in the existing CRM Activities system, explicitly linked to this opportunity, assigned to its authorized owner and given a due date.', { avoid: 'Do not create an unrelated Lead activity to satisfy the Negotiation gate.', sopReference: 'Part 11 Next Action' }),
