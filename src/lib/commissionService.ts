...
function mapRule(row: any, index = 0): CommissionRule {
  return {
    ...
    selfGeneratedRatePercent: Number(row.self_generated_rate_percent ?? row.base_rate_percent ?? 0),
...
async function persistRule(rule: CommissionRule): Promise<void> {
  const { error } = await supabase.from('commission_rules').upsert({
    ...
    self_generated_rate_percent: rule.selfGeneratedRatePercent,
...
export const commissionService = {
  ...
  saveRule(rule: Partial<CommissionRule>, userEmail: string, persist = true): CommissionRule {
    ...
    const saved: CommissionRule = {
      ...
      selfGeneratedRatePercent: Number(rule.selfGeneratedRatePercent ?? previous?.selfGeneratedRatePercent ?? rule.baseRatePercent ?? previous?.baseRatePercent ?? 10),
