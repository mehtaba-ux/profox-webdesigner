...
function readChatTokens(): ChatTokenMap { return readJson<ChatTokenMap>(LOCAL_CHAT_TOKENS_KEY, {}); }
...
function chatTokenFor(conversationId: string) { return readChatTokens()[conversationId]?.token || ''; }
export const getChatAccessToken = (conversationId: string) => chatTokenFor(conversationId);

function asArray<T>(value: unknown): T[] { return Array.isArray(value) ? value as T[] : []; }
function rpcError(error: any, fallback: string) { return new Error(error?.message || fallback); }
...
function isRecoveryChallenge(value: any): value is ChatRecoveryChallenge { return Boolean(value?.verificationRequired && value?.challengeId); }
...
async function getPublicTokenConversations(): Promise<ChatConversationResult[]> {
  ...
  const conversations = await Promise.all(Object.entries(records).map(async ([conversationId, record]) => {
    ...
    const { data, error } = await supabase.rpc('public_sales_chat_get_conversation', { p_conversation_id: conversationId, p_access_token: record.token });
...
export const playChatChime = () => {
  try {
    ...
    const context = new AudioContext(); const now = context.currentTime; const oscillator = context.createOscillator(); const gain = context.createGain();
    oscillator.type = 'sine'; oscillator.frequency.setValueAtTime(523.25, now); oscillator.frequency.exponentialRampToValueAtTime(659.25, now + 0.12);
  ...
  } catch (error) { console.warn('Audio chime unsupported:', error); }
};
...
export const startSalesCustomerChat = async (input: { leadId: string; message: string }): Promise<SalesStartedChatResult> => {
  ...
  const { data, error } = await supabase.rpc('sales_start_customer_chat', { p_lead_id: input.leadId, p_message: message });
...
export const redeemSalesChatLink = async (linkToken: string): Promise<ChatConversationResult> => {
  ...
  const { data, error } = await supabase.rpc('public_sales_chat_redeem_link', { p_link_token: linkToken, p_access_token: accessToken });
...
export const getSalesReps = async (): Promise<SalesRep[]> => {
  ...
  const reps = asArray<SalesRep>(data).map(rep => ({ ...rep, email: '', avatar: rep.avatar || '', specialties: Array.isArray(rep.specialties) ? rep.specialties : [], rating: Number(rep.rating ?? 0), reviewCount: Number(rep.reviewCount || 0), isOnline: Boolean(rep.isOnline) })).filter(rep => UUID_PATTERN.test(rep.id));
...
export const getConversations = async (): Promise<ChatConversationResult[]> => {
  if (await hasSignedInSession()) {
    ...
    if (!/sales chat access required/i.test(error.message || '')) throw rpcError(error, 'The sales inbox could not be loaded.');
  }
...
export const getOriginalSalesAssignment = async (email: string): Promise<{ originalRep: SalesRep | null; previousConversation: ChatConversationResult | null }> => {
  if (!email) return { originalRep: null, previousConversation: null };
  const normalizedEmail = email.trim().toLowerCase();
  const conversations = (await getPublicTokenConversations()).filter(conversation => conversation.customerEmail.toLowerCase() === normalizedEmail);
  ...
  const oldest = sorted[0]; const reps = await getSalesReps();
  const rep = reps.find(item => item.id === oldest.currentSalesId) || reps.find(item => item.id === oldest.originalSalesId) || null;
...
export const requestChatRecovery = async (email: string): Promise<ChatRecoveryChallenge> => {
  ...
  return { verificationRequired: true, challengeId: String(data.challengeId), expiresAt: data.expiresAt ? String(data.expiresAt) : undefined, delivery: 'email' };
};

export const verifyChatRecovery = async (input: { challengeId: string; code: string; email: string }): Promise<ChatConversationResult> => {
  const accessToken = crypto.randomUUID();
  const { data, error } = await supabase.rpc('public_sales_chat_verify_recovery', { p_challenge_id: input.challengeId, p_code: input.code.trim().toUpperCase(), p_access_token: accessToken });
...
export const createOrGetConversation = async (input: { customerName: string; customerEmail: string; customerPhone?: string; intent: 'new_package' | 'existing_issue'; selectedSalesId?: string }): Promise<ChatOpenResult> => {
  const email = input.customerEmail.trim().toLowerCase();
  const existing = (await getPublicTokenConversations()).filter(conversation => conversation.customerEmail.toLowerCase() === email).filter(conversation => conversation.conversationKind !== 'quotation').sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime())[0];
  if (existing) return { ...existing, verificationRequired: false, relationshipResumed: true };
  const accessToken = crypto.randomUUID();
  const payload: Record<string, string> = { customerName: input.customerName.trim(), customerEmail: email, customerPhone: input.customerPhone?.trim() || '', intent: input.intent };
  if (input.selectedSalesId && UUID_PATTERN.test(input.selectedSalesId)) payload.selectedSalesId = input.selectedSalesId;
  const { data, error } = await supabase.rpc('public_sales_chat_open', { p_payload: payload, p_access_token: accessToken });
...
export const getChatConversation = async (conversationId: string): Promise<ChatConversationResult> => {
  ...
  const { data, error } = await supabase.rpc('public_sales_chat_get_conversation', { p_conversation_id: conversationId, p_access_token: publicToken });
...
export const getChatMessages = async (conversationId: string): Promise<ChatMessage[]> => {
  const publicToken = chatTokenFor(conversationId);
  if (publicToken) {
    const { data, error } = await supabase.rpc('public_sales_chat_get_messages', { p_conversation_id: conversationId, p_access_token: publicToken });
  ...
  if (!/sales_client_inbox_timeline|function .* does not exist/i.test(error.message || '')) throw rpcError(error, 'Messages could not be loaded.');
...
export const sendChatMessage = async (input: {
  ...
  const attachmentIds = (input.attachmentIds || []).filter(id => UUID_PATTERN.test(id));
  if (!message && attachmentIds.length === 0) throw new Error('Enter a message or attach a file before sending.');
  ...
  if (input.senderType === 'customer') {
    ...
    const { data, error } = attachmentIds.length
      ? await supabase.rpc('public_sales_chat_send_message_with_attachments', { p_conversation_id: input.conversationId, p_access_token: accessToken, p_message: message, p_attachment_ids: attachmentIds })
      : await supabase.rpc('public_sales_chat_send_message', { p_conversation_id: input.conversationId, p_access_token: accessToken, p_message: message });
  ...
  const { data, error } = attachmentIds.length
    ? await supabase.rpc('sales_chat_send_message_with_attachments', { p_conversation_id: input.conversationId, p_message: message, p_internal_note: Boolean(input.isInternalNote), p_attachment_ids: attachmentIds })
    : await supabase.rpc('sales_chat_send_message', { p_conversation_id: input.conversationId, p_message: message, p_internal_note: Boolean(input.isInternalNote) });
...
export const submitChatRating = async (conversationId: string, rating: number, feedback = ''): Promise<void> => {
  ...
  if (!accessToken) throw new Error('This browser is not authorized for the conversation.');
  const { error } = await supabase.rpc('public_sales_chat_submit_rating', { p_conversation_id: conversationId, p_access_token: accessToken, p_rating: rating, p_feedback: feedback });
...
export const resolvePublicChat = async (conversationId: string): Promise<void> => {
  const accessToken = chatTokenFor(conversationId);
  if (!accessToken) throw new Error('This browser is not authorized for the conversation.');
  const { error } = await supabase.rpc('public_sales_chat_resolve', { p_conversation_id: conversationId, p_access_token: accessToken });
  if (error) throw rpcError(error, 'The conversation could not be closed.');
};

export const updateConversationStatus = async (conversationId: string, status: 'open' | 'pending' | 'resolved'): Promise<void> => {
  const { error } = await supabase.rpc('sales_chat_update_status', { p_conversation_id: conversationId, p_status: status });
