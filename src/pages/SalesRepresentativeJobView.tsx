...
function ApplicationSection({ context }: { context: PublicSalesRoleContext }) {
  ...
  const [step,setStep]=useState(0);const[submitted,setSubmitted]=useState(false);const[reference,setReference]=useState('');const[submitting,setSubmitting]=useState(false);const[error,setError]=useState('');const[fieldErrors,setFieldErrors]=useState<Record<string,string>>({});
  const [uploading,setUploading]=useState<'cv'|'video'|''>('');const[uploadProgress,setUploadProgress]=useState(0);const[cvName,setCvName]=useState('');const[videoName,setVideoName]=useState('');
  const initialTimezone=(()=>{try{return Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC'}catch{return'UTC'}})();
  const [form,setForm]=useState({fullName:'',email:'',phone:'',country:'',countryCode:'',timezone:initialTimezone,currentRole:'',linkedinUrl:'',salesExperienceMonths:'',b2bExperienceMonths:'',salesExperience:'',digitalSalesExperience:'',internationalSalesExperience:'',previousSalesResults:'',targetMarkets:[] as string[],prospectingChannels:[] as string[],crmExperience:'',englishRating:'',availableDays:[] as string[],availableHoursPerWeek:'',preferredWorkWindow:'',earliestStartDate:'',sampleOutreachMessage:'',professionalReference:'',heardAboutSource:'',heardAboutDetail:'',message:'',cvUrl:'',videoUrl:'',cvStoragePath:'',videoStoragePath:'',hasLaptopInternet:false,comfortableCommission:false,comfortableSourcing:false,comfortableEnglishCalls:false,videoCommitment:false,consentAccurate:false,consentPrivacy:false});
  const patch=(key:string,value:any)=>{setForm(current=>({...current,[key]:value}));setFieldErrors(current=>{const n={...current};delete n[key];return n})};
  ...
  const validateStep=(value:number)=>{setError('');setFieldErrors({});
    ...
    if(value===4){if(!form.cvStoragePath&&!/^https?:\/\//i.test(form.cvUrl.trim()))return fail('cv','Upload your CV/resume or provide a shareable link.');if(!form.videoStoragePath&&!/^https?:\/\//i.test(form.videoUrl.trim()))return fail('video','Upload your introduction video or provide a shareable link.');if(!form.consentAccurate||!form.consentPrivacy)return fail('consent','Review and accept the required confirmations.');}
    return true;
  };
  ...
  const uploadFile=async(kind:'cv'|'video',file?:File)=>{if(!file)return;setError('');if(!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(form.email.trim())){setError('Enter a valid email in Personal Details before uploading files.');return;}setUploading(kind);setUploadProgress(0);try{const result=await applicantService.uploadPublicApplicationFile(form.email,kind,file,setUploadProgress);patch(kind==='cv'?'cvStoragePath':'videoStoragePath',result.path);if(kind==='cv')setCvName(file.name);else setVideoName(file.name);}catch(err:any){setError(err?.message||'The file could not be uploaded. You can also use a shareable link.');}finally{setUploading('');}};
  const submit=async(e:React.FormEvent)=>{e.preventDefault();if(step<4){next();return;}if(!validateStep(4))return;setSubmitting(true);setError('');try{const result=await applicantService.submitSalesApplicationV3({...form,salesExperienceMonths:Number(form.salesExperienceMonths),b2bExperienceMonths:form.b2bExperienceMonths?Number(form.b2bExperienceMonths):null,availableHoursPerWeek:Number(form.availableHoursPerWeek)});if(result.success){setReference(result.reference||'');setSubmitted(true);}else{if(result.field)setFieldErrors({[result.field]:result.error||result.message||'Please review this field.'});setError(result.message||result.error||'Your application could not be submitted.');}}catch(err:any){setError(err?.message||'Your application could not be submitted.');}finally{setSubmitting(false);}};
  const countries=countryOptions();const timezones=timezoneOptions();
  const steps=['Eligibility','Personal details','Sales evidence','Availability & practical','Proof & submit'];
  return <section id="apply" className="scroll-mt-28 border-t border-slate-200 bg-[#f4f5fb] py-20 sm:py-24"><div className="pf-container"><div className="mx-auto max-w-4xl"><SectionIntro eyebrow="Apply" title="Apply for this role" text="A structured application helps us review every candidate consistently. Your answers become the same candidate record used throughout Recruitment." />
    {submitted?<div className="mt-10 rounded-2xl border border-emerald-200 bg-emerald-50 p-8 text-center"><CheckCircle2 className="mx-auto h-10 w-10 text-emerald-600"/><h3 className="mt-4 text-xl font-semibold text-slate-900">Application received.</h3>{reference&&<div className="mx-auto mt-4 w-fit rounded-lg border border-emerald-200 bg-white px-4 py-2 text-sm font-bold text-emerald-800">Reference: {reference}</div>}<p className="mx-auto mt-4 max-w-lg text-sm leading-6 text-slate-600">Your application has entered Video Review. Keep your reference for any future application-related communication.</p></div>:<form onSubmit={submit} className="mt-10 overflow-hidden rounded-[24px] border border-slate-200 bg-white shadow-[0_24px_70px_rgba(15,23,42,0.08)]">
      <div className="border-b border-slate-200 bg-[#fbfcff] px-5 py-5 sm:px-8"><div className="grid grid-cols-5 gap-2">{steps.map((label,index)=><button type="button" key={label} onClick={()=>index<step&&setStep(index)} className="text-left"><div className={`h-1.5 rounded-full ${index<=step?'bg-[#000080]':'bg-slate-200'}`}/><div className={`mt-2 hidden text-[10px] font-bold uppercase tracking-[0.08em] sm:block ${index===step?'text-[#000080]':'text-slate-400'}`}>{label}</div></button>)}</div><div className="mt-3 text-xs font-bold text-slate-500 sm:hidden">Step {step+1} of 5 · {steps[step]}</div></div>
      <div className="space-y-7 p-6 sm:p-8">{error&&<div className="rounded-xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</div>}
        {step===0&&<><StepTitle title="Confirm the role fits before you apply" text="These are current minimum requirements from the published Sales role. If they do not work for you today, you can avoid spending time on the full application."/><div className="grid gap-4 sm:grid-cols-2"><Field label="Total sales experience in months *" error={fieldErrors.salesExperienceMonths} hint={`Minimum currently required: ${minExperience} months`}><input type="number" min={0} max={600} className={inputClass} value={form.salesExperienceMonths} onChange={e=>patch('salesExperienceMonths',e.target.value)}/></Field><Field label="Hours available per week *" error={fieldErrors.availableHoursPerWeek} hint={`Minimum currently required: ${minHours} hours`}><input type="number" min={1} max={80} className={inputClass} value={form.availableHoursPerWeek} onChange={e=>patch('availableHoursPerWeek',e.target.value)}/></Field></div><div className="space-y-3 rounded-2xl border border-slate-200 bg-[#fbfcff] p-5"><Confirm checked={form.comfortableCommission} onChange={v=>patch('comfortableCommission',v)}>I understand this role starts as an independent commission-based opportunity.</Confirm><Confirm checked={form.comfortableSourcing} onChange={v=>patch('comfortableSourcing',v)}>I can initially research and generate my own qualified leads.</Confirm><Confirm checked={form.comfortableEnglishCalls} onChange={v=>patch('comfortableEnglishCalls',v)}>I can conduct professional sales conversations and video meetings in English.</Confirm><Confirm checked={form.hasLaptopInternet} onChange={v=>patch('hasLaptopInternet',v)}>I have a computer, reliable internet, webcam and a suitable call environment.</Confirm><Confirm checked={form.videoCommitment} onChange={v=>patch('videoCommitment',v)}>I can submit the required 60-120 second English introduction video.</Confirm>{fieldErrors.eligibility&&<p className="text-xs font-bold text-red-600">{fieldErrors.eligibility}</p>}</div></>}
        {step===1&&<><StepTitle title="Personal and contact details" text="Use details that management can rely on throughout screening and onboarding."/><div className="grid gap-4 sm:grid-cols-2"><Field label="Full name *" error={fieldErrors.fullName}><input className={inputClass} autoComplete="name" value={form.fullName} onChange={e=>patch('fullName',e.target.value)}/></Field><Field label="Email *" error={fieldErrors.email}><input type="email" className={inputClass} autoComplete="email" value={form.email} onChange={e=>patch('email',e.target.value)}/></Field><Field label="WhatsApp / contact number *" error={fieldErrors.phone} hint="Include country code, for example +14155552671"><input type="tel" className={inputClass} autoComplete="tel" value={form.phone} onChange={e=>patch('phone',e.target.value)}/></Field><Field label="Country *" error={fieldErrors.country}><select className={inputClass} value={form.countryCode} onChange={e=>{const item=countries.find(x=>x.code===e.target.value);patch('countryCode',e.target.value);patch('country',item?.name||'')}}><option value="">Select country</option>{countries.map(item=><option key={item.code} value={item.code}>{item.name}</option>)}</select></Field><Field label="Time zone *" error={fieldErrors.timezone}><select className={inputClass} value={form.timezone} onChange={e=>patch('timezone',e.target.value)}>{timezones.map(item=><option key={item} value={item}>{item}</option>)}</select></Field><Field label="Current role"><input className={inputClass} value={form.currentRole} onChange={e=>patch('currentRole',e.target.value)}/></Field></div><Field label="LinkedIn profile URL *" error={fieldErrors.linkedinUrl}><input className={inputClass} value={form.linkedinUrl} onChange={e=>patch('linkedinUrl',e.target.value)} placeholder="https://linkedin.com/in/..."/></Field></>}
        {step===2&&<><StepTitle title="Show us how you sell" text="We are looking for evidence and judgment, not polished buzzwords."/><div className="grid gap-4 sm:grid-cols-2"><Field label="English level *" error={fieldErrors.englishRating}><select className={inputClass} value={form.englishRating} onChange={e=>patch('englishRating',e.target.value)}><option value="">Select level</option><option>Fluent / Native</option><option>Professional working proficiency</option><option>Conversational</option></select></Field><Field label="B2B sales experience in months"><input type="number" min={0} max={600} className={inputClass} value={form.b2bExperienceMonths} onChange={e=>patch('b2bExperienceMonths',e.target.value)}/></Field></div><Field label="Sales experience *" error={fieldErrors.salesExperience}><textarea rows={4} className={inputClass} value={form.salesExperience} onChange={e=>patch('salesExperience',e.target.value)} placeholder="What have you sold, to whom, and what parts of the sales cycle did you own?"/></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="Digital / SaaS / website sales experience"><textarea rows={3} className={inputClass} value={form.digitalSalesExperience} onChange={e=>patch('digitalSalesExperience',e.target.value)}/></Field><Field label="International sales experience"><textarea rows={3} className={inputClass} value={form.internationalSalesExperience} onChange={e=>patch('internationalSalesExperience',e.target.value)}/></Field></div><Field label="One measurable sales result *" error={fieldErrors.previousSalesResults} hint="Tell us what you sold, approximate deal value/result, and your contribution."><textarea rows={4} className={inputClass} value={form.previousSalesResults} onChange={e=>patch('previousSalesResults',e.target.value)}/></Field><ChoiceGroup title="Markets you have sold to *" options={marketOptions} selected={form.targetMarkets} onToggle={v=>toggle('targetMarkets',v)} error={fieldErrors.targetMarkets}/><ChoiceGroup title="Prospecting / sales channels you have used *" options={channelOptions} selected={form.prospectingChannels} onToggle={v=>toggle('prospectingChannels',v)} error={fieldErrors.prospectingChannels}/><Field label="CRM experience"><textarea rows={3} className={inputClass} value={form.crmExperience} onChange={e=>patch('crmExperience',e.target.value)} placeholder="Which CRM systems have you used and how did you use them?"/></Field></>}
        {step===3&&<><StepTitle title="Availability and practical evidence" text="This helps us compare availability and selling judgment without an extra screening round."/><ChoiceGroup title="Days you can normally work *" options={['Monday','Tuesday','Wednesday','Thursday','Friday','Saturday','Sunday']} selected={form.availableDays} onToggle={v=>toggle('availableDays',v)} error={fieldErrors.availableDays}/><div className="grid gap-4 sm:grid-cols-2"><Field label="Hours available per week *" error={fieldErrors.availableHoursPerWeek}><input type="number" min={1} max={80} className={inputClass} value={form.availableHoursPerWeek} onChange={e=>patch('availableHoursPerWeek',e.target.value)}/></Field><Field label="Earliest start date *" error={fieldErrors.earliestStartDate}><input type="date" min={new Date().toISOString().slice(0,10)} className={inputClass} value={form.earliestStartDate} onChange={e=>patch('earliestStartDate',e.target.value)}/></Field></div><Field label="Preferred working window"><input className={inputClass} value={form.preferredWorkWindow} onChange={e=>patch('preferredWorkWindow',e.target.value)} placeholder="Example: 2 PM-10 PM local time"/></Field><Field label="Sample cold outreach message *" error={fieldErrors.sampleOutreachMessage} hint="Write a short message you would send to a qualified business prospect."><textarea rows={5} className={inputClass} value={form.sampleOutreachMessage} onChange={e=>patch('sampleOutreachMessage',e.target.value)}/></Field><div className="grid gap-4 sm:grid-cols-2"><Field label="How did you hear about ProFox? *" error={fieldErrors.heardAboutSource}><select className={inputClass} value={form.heardAboutSource} onChange={e=>patch('heardAboutSource',e.target.value)}><option value="">Select source</option>{sourceOptions.map(item=><option key={item}>{item}</option>)}</select></Field><Field label="Source details"><input className={inputClass} value={form.heardAboutDetail} onChange={e=>patch('heardAboutDetail',e.target.value)} placeholder="Job board name, referrer, page, etc."/></Field></div></>}
        {step===4&&<><StepTitle title="Proof, introduction video and consent" text="Files are stored privately. You may upload directly or provide a shareable link instead."/><div className="grid gap-5 lg:grid-cols-2"><UploadCard kind="cv" title="CV / Resume *" accept=".pdf,.doc,.docx" maxLabel="PDF, DOC or DOCX · max 8 MB" uploadedName={cvName} uploading={uploading==='cv'} progress={uploading==='cv'?uploadProgress:0} onFile={file=>void uploadFile('cv',file)} /><UploadCard kind="video" title="Introduction video *" accept="video/mp4,video/webm,video/quicktime" maxLabel="MP4, WebM or MOV · max 80 MB" uploadedName={videoName} uploading={uploading==='video'} progress={uploading==='video'?uploadProgress:0} onFile={file=>void uploadFile('video',file)} /></div><div className="grid gap-4 lg:grid-cols-2"><Field label="Or CV / resume shareable link" error={fieldErrors.cv||fieldErrors.cvUrl}><input className={inputClass} value={form.cvUrl} onChange={e=>patch('cvUrl',e.target.value)} placeholder="https://..."/></Field><Field label="Or introduction video shareable link" error={fieldErrors.video||fieldErrors.videoUrl}><input className={inputClass} value={form.videoUrl} onChange={e=>patch('videoUrl',e.target.value)} placeholder="Loom, Drive, YouTube unlisted, etc."/></Field></div><Field label="Professional reference (optional)"><textarea rows={3} className={inputClass} value={form.professionalReference} onChange={e=>patch('professionalReference',e.target.value)} placeholder="Name, relationship and contact details where appropriate."/></Field><Field label="Why ProFox / additional context"><textarea rows={4} className={inputClass} value={form.message} onChange={e=>patch('message',e.target.value)}/></Field><div className="space-y-3 rounded-2xl border border-slate-200 bg-[#fbfcff] p-5"><Confirm checked={form.consentAccurate} onChange={v=>patch('consentAccurate',v)}>I confirm the information and links in this application are accurate.</Confirm><Confirm checked={form.consentPrivacy} onChange={v=>patch('consentPrivacy',v)}>I consent to ProFox using this information and submitted files to evaluate and manage my application. <Link to="/privacy-policy" className="font-bold text-[#000080] underline underline-offset-2">Privacy Policy</Link></Confirm>{fieldErrors.consent&&<p className="text-xs font-bold text-red-600">{fieldErrors.consent}</p>}</div></>}
      </div>
      <div className="flex items-center justify-between gap-3 border-t border-slate-200 bg-[#fbfcff] px-6 py-5 sm:px-8">{step>0?<button type="button" onClick={back} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-xs font-bold text-slate-600 transition hover:border-slate-300"><ChevronLeft className="h-4 w-4"/> Back</button>:<span/>}{step<4?<button type="button" onClick={next} className="inline-flex items-center gap-2 rounded-lg bg-[#000080] px-5 py-2.5 text-xs font-bold text-white transition hover:bg-[#000066]">Continue <ChevronRight className="h-4 w-4"/></button>:<button disabled={submitting||Boolean(uploading)} className="inline-flex items-center gap-2 rounded-lg bg-[#000080] px-5 py-2.5 text-xs font-bold text-white transition hover:bg-[#000066] disabled:opacity-50">{submitting?<Loader2 className="h-4 w-4 animate-spin"/>:<Send className="h-4 w-4"/>} Submit application</button>}</div>
    </form>}
  </div></div></section>;
}

const inputClass='w-full rounded-lg border border-slate-300 bg-white px-3.5 py-3 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-[#000080]/50 focus:ring-4 focus:ring-[#000080]/5';
...
function StepTitle({title,text}:{title:string;text:string}){return <div><h3 className="text-2xl font-semibold tracking-[-0.025em] text-[#071126]">{title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{text}</p></div>}
...
function Field({label,children,error,hint}:{label:string;children:any;error?:string;hint?:string}){return <label className="block text-sm font-semibold text-slate-700">{label}<div className="mt-2">{children}</div>{hint&&<div className="mt-1.5 text-xs font-normal leading-5 text-slate-400">{hint}</div>}{error&&<div className="mt-1.5 text-xs font-semibold text-red-600">{error}</div>}</label>}
function Confirm({checked,onChange,children}:{checked:boolean;onChange:(v:boolean)=>void;children:any}){return <label className="flex items-start gap-3 text-sm leading-6 text-slate-600"><input type="checkbox" checked={checked} onChange={e=>onChange(e.target.checked)} className="mt-1 h-4 w-4 accent-[#000080]"/><span>{children}</span></label>}
function ChoiceGroup({title,options,selected,onToggle,error}:{title:string;options:string[];selected:string[];onToggle:(v:string)=>void;error?:string}){return <div><div className="text-sm font-semibold text-slate-700">{title}</div><div className="mt-3 flex flex-wrap gap-2">{options.map(item=><button type="button" key={item} onClick={()=>onToggle(item)} className={`rounded-full border px-3 py-2 text-xs font-semibold transition ${selected.includes(item)?'border-[#000080] bg-[#000080]/5 text-[#000080]':'border-slate-200 bg-white text-slate-600 hover:border-slate-300'}`}>{selected.includes(item)&&<CheckCircle2 className="mr-1.5 inline h-3.5 w-3.5"/>}{item}</button>)}</div>{error&&<div className="mt-2 text-xs font-semibold text-red-600">{error}</div>}</div>}
function UploadCard({kind,title,accept,maxLabel,uploadedName,uploading,progress,onFile}:{kind:string;title:string;accept:string;maxLabel:string;uploadedName:string;uploading:boolean;progress:number;onFile:(file?:File)=>void}){return <label className={`block cursor-pointer rounded-2xl border border-dashed p-5 transition ${uploadedName?'border-emerald-300 bg-emerald-50':'border-slate-300 bg-[#fbfcff] hover:border-[#000080]/40'}`}><input type="file" accept={accept} className="hidden" disabled={uploading} onChange={e=>onFile(e.target.files?.[0])}/><div className="flex items-start gap-3"><div className={`rounded-xl p-2 ${uploadedName?'bg-emerald-100 text-emerald-700':'bg-white text-[#000080] shadow-sm'}`}><UploadCloud className="h-5 w-5"/></div><div className="min-w-0"><div className="text-sm font-semibold text-slate-800">{title}</div><div className="mt-1 text-xs text-slate-400">{uploadedName||maxLabel}</div>{uploading&&<div className="mt-3"><div className="h-1.5 overflow-hidden rounded-full bg-slate-200"><div className="h-full bg-[#000080] transition-all" style={{width:`${progress}%`}}/></div><div className="mt-1 text-[10px] font-bold text-slate-400">Uploading {kind}...</div></div>}{uploadedName&&!uploading&&<div className="mt-2 text-[10px] font-bold text-emerald-700">Secure upload complete. Choose another file to replace it before submission.</div>}</div></div></label>}
