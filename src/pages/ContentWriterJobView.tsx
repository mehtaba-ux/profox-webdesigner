...
import { ArrowLeft, ArrowRight, CheckCircle2, FileText, Loader2, ShieldCheck, UploadCloud } from 'lucide-react';
...
type FormState = {
  ...
  portfolioUrl:string;sampleOne:string;sampleTwo:string;contentExperience:string;researchApproach:string;
  qualityProcess:string;skills:string;availableHoursPerWeek:string;weeklyAvailability:string;earliestStartDate:string;
  motivation:string;hasLaptopInternet:boolean;consentAccurate:boolean;consentPrivacy:boolean;
};

const initial:FormState={fullName:'',email:'',phone:'',country:'',linkedinUrl:'',currentRole:'',portfolioUrl:'',sampleOne:'',sampleTwo:'',contentExperience:'',researchApproach:'',qualityProcess:'',skills:'',availableHoursPerWeek:'20',weeklyAvailability:'',earliestStartDate:'',motivation:'',hasLaptopInternet:false,consentAccurate:false,consentPrivacy:false};
...
export default function ContentWriterJobView({job}:Props){
  ...
  const samples=useMemo(()=>[form.sampleOne,form.sampleTwo].map(v=>v.trim()).filter(Boolean),[form.sampleOne,form.sampleTwo]);
  ...
  const submit=async(event:FormEvent)=>{
    ...
    if(!form.portfolioUrl.trim()&&samples.length===0){setError('Add a portfolio or at least one writing sample.');return;}
    ...
    setBusy(true);
    try{
      const uploaded=await applicantService.uploadPublicApplicationFile(form.email.trim().toLowerCase(),'cv',cv,setProgress);
      const {data,rpcError}=await (async()=>{const result=await supabase.rpc('submit_public_content_writer_application',{p_application:{
        ...form,
        fullName:form.fullName.trim(),email:form.email.trim().toLowerCase(),timezone:Intl.DateTimeFormat().resolvedOptions().timeZone||'UTC',
        availableHoursPerWeek:Number(form.availableHoursPerWeek),writingSamples:samples,cvStoragePath:uploaded.path
      }});return{data:result.data,rpcError:result.error}})();
      if(rpcError)throw rpcError;
      if(!data?.success)throw new Error(data?.error||'Your application could not be submitted.');
  ...
  return <div className="min-h-screen bg-white text-slate-900">
    ...
    <section id="apply" className="border-t border-slate-200 bg-slate-50 py-16"><form onSubmit={submit} className="mx-auto max-w-4xl space-y-7 px-6"><div><Eyebrow>Application</Eyebrow><h2 className="mt-3 text-3xl font-black">Apply to the Content team</h2><p className="mt-2 text-sm text-slate-500">Give us enough evidence to evaluate your writing, research discipline and quality process. Required information is checked before review.</p></div>{error&&<div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm font-semibold text-red-700">{error}</div>}
      ...
      <Card title="Writing evidence"><Grid><Field label="Portfolio URL"><input type="url" value={form.portfolioUrl} onChange={e=>set('portfolioUrl',e.target.value)}/></Field><Field label="Writing sample URL 1"><input type="url" value={form.sampleOne} onChange={e=>set('sampleOne',e.target.value)}/></Field><Field label="Writing sample URL 2"><input type="url" value={form.sampleTwo} onChange={e=>set('sampleTwo',e.target.value)}/></Field><Field label="CV / Resume *"><label className="flex cursor-pointer items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50 p-3 text-xs font-bold text-slate-600"><UploadCloud className="h-4 w-4 text-[#000080]"/>{cv?cv.name:'Upload PDF, DOC or DOCX'}<input className="hidden" type="file" accept=".pdf,.doc,.docx" onChange={e=>setCv(e.target.files?.[0]||null)}/></label>{busy&&progress>0&&<div className="mt-2 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full bg-[#000080]" style={{width:`${progress}%`}}/></div>}</Field></Grid></Card>
