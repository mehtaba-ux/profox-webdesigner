...
export default function TalentPartnerPayoutSetup() {
  ...
  const load = async () => {
    ...
    if (rpcError) setError(rpcError.message || 'Could not load payout setup.');
      ...
      setEditing(next.status === 'Setup Required' || next.status === 'Rejected');
    }
    setLoading(false);
  };
  ...
  const submit = async (e:FormEvent) => {
    ...
    try {
      ...
      setState(data as SetupState);
      clearSensitive(); setEditing(false);
      setMessage('Payout details submitted securely for verification.');
    } catch (err:any) {
  ...
  const statusTone = state.status === 'Verified' ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : state.status === 'Rejected' ? 'text-red-700 bg-red-50 border-red-200' : 'text-amber-800 bg-amber-50 border-amber-200';

  return <div className="min-h-screen bg-[#f5f7fb] px-4 py-8 text-slate-900 sm:px-6 lg:px-8">
    <div className="mx-auto max-w-5xl">
      <div className="mb-6 flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div><div className="text-[10px] font-black uppercase tracking-[.16em] text-[#FF0E0E]">Talent Partner · Secure payout</div><h1 className="mt-2 text-3xl font-black tracking-[-.04em] text-[#071126]">Payout setup & verification</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">Sensitive payout details are requested only after you have an approved reward. They are encrypted at rest and are not shown in the normal partner profile.</p></div>
      ...
      <div className="grid gap-6 lg:grid-cols-[.8fr_1.2fr]">
        <div className="space-y-6">
          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {state.approvedBalances?.length ? <div className="mt-5 space-y-2"><div className="text-[10px] font-black uppercase text-slate-400">Approved, unpaid rewards</div>{state.approvedBalances.map(x=><div key={x.currency} className="flex justify-between rounded-xl bg-slate-50 p-3 text-sm"><span>{x.currency}</span><strong>{Number(x.amount||0).toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}</strong></div>)}</div> : <p className="mt-4 text-xs leading-5 text-slate-500">There is currently no approved unpaid reward requiring payout setup.</p>}
          </section>

          <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex items-center gap-2 text-sm font-black"><LockKeyhole className="h-4 w-4 text-[#000080]"/>Security</div>
            <div className="mt-4 space-y-3 text-xs leading-5 text-slate-600"><Row icon={ShieldCheck} text="Full account details are encrypted in Supabase and never returned to the partner dashboard."/><Row icon={Clock3} text="Admin access to decrypted details is audited and requires an explicit reason."/><Row icon={CheckCircle2} text="Changing verified details resets verification before another payout can use them."/></div>
          </section>
        </div>

        <section className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          {!state.setupRequired && !hasProfile ? <div className="py-8 text-center"><WalletCards className="mx-auto h-10 w-10 text-[#000080]"/><h2 className="mt-4 text-xl font-black">No payout setup required yet</h2><p className="mx-auto mt-3 max-w-lg text-sm leading-6 text-slate-500">You do not need to provide financial account details during registration. This page becomes active after ProFox approves a reward for payout.</p></div> : !editing ? <div>
            ...
            {state.status==='Verification Pending'&&<Notice tone="info">ProFox must review these details before any approved reward can enter a payout batch.</Notice>}
            ...
            <button onClick={()=>{clearSensitive();setEditing(true)}} className="light-button mt-4">{state.status==='Rejected'?'Correct payout details':'Change payout details'}</button>
          </div> : <form onSubmit={submit} className="space-y-5">
            <div><div className="text-[10px] font-black uppercase tracking-[.14em] text-[#000080]">Secure details</div><h2 className="mt-2 text-2xl font-black">{hasProfile?'Update payout profile':'Set up your payout profile'}</h2><p className="mt-2 text-sm leading-6 text-slate-500">Enter the complete account information again when changing a profile. Previously stored sensitive values are intentionally never sent back to your browser.</p></div>
            ...
            {method==='PayPal'&&<><Notice tone="warn"><strong>India limitation:</strong> ProFox cannot use standard PayPal Payouts API for automated/bulk outbound payouts from an India merchant account. PayPal is therefore manually verified and manually settled, with the transaction reference recorded in ProFox.</Notice><Field label="PayPal email"><input required type="email" autoComplete="off" value={details.paypalEmail} onChange={e=>patch('paypalEmail',e.target.value)}/></Field></>}
            ...
            <label className="flex items-start gap-3 rounded-xl border border-blue-100 bg-blue-50/50 p-4 text-xs leading-5 text-slate-600"><input required type="checkbox" className="mt-0.5 h-4 w-4 accent-[#000080]"/><span>I confirm these payout details belong to me or my authorized business account and are accurate. I understand ProFox will verify them before payout.</span></label>
            <div className="flex flex-col gap-2 sm:flex-row"><button disabled={saving} className="primary">{saving?<Loader2 className="h-4 w-4 animate-spin"/>:<CreditCard className="h-4 w-4"/>}{saving?'Encrypting & submitting...':'Submit for verification'}</button>{hasProfile&&<button type="button" onClick={()=>{clearSensitive();setEditing(false)}} className="light-button">Cancel</button>}</div>
          </form>}
        </section>
      </div>
    </div>
