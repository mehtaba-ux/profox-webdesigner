...
function dateKey(iso: string, tz: string) {
  const p = new Intl.DateTimeFormat('en-US', { timeZone: tz, year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(new Date(iso));
...
export default function BookingManagementPage() {
  ...
  const grouped = useMemo(() => {
    ...
    for (const slot of slots) {
      const key = dateKey(slot.startAt, visitorTimezone);
  ...
  const load = async () => {
    ...
    try {
      let next = await bookingManagementService.get(token);
      if (searchParams.get('confirm') === '1' && next.status === 'Confirmed' && next.attendanceStatus !== 'Confirmed') {
        next = await bookingManagementService.confirmAttendance(token);
        setSuccess('Attendance confirmed. We will see you at the scheduled time.');
      ...
      setError(err?.message || 'This booking link is unavailable.');
    } finally { setLoading(false); }
  };

  const loadSlots = async (start = localIsoDate()) => {
    setSlotsLoading(true); setError('');
    try {
      const rows = await bookingManagementService.slots(token, start, 14);
      setSlots(rows); setFromDate(start);
      setSelectedDate(rows[0] ? dateKey(rows[0].startAt, visitorTimezone) : '');
    } catch (err: any) {
      setError(err?.message || 'Availability could not be loaded.'); setSlots([]);
  ...
  const confirmAttendance = async () => {
    ...
    try {
      const updated = await bookingManagementService.confirmAttendance(token);
      setBooking(updated);
      setSuccess('Attendance confirmed. We will see you at the scheduled time.');
    } catch (err: any) { setError(err?.message || 'Attendance could not be confirmed.'); }
    finally { setSaving(false); }
  };

  const reschedule = async (slot: BookingSlot) => {
    setSaving(true); setError(''); setSuccess('');
    try {
      const updated = await bookingManagementService.reschedule(token, slot.startAt, visitorTimezone);
      setBooking(updated);
      setSuccess('Your meeting has been rescheduled. Your ProFox specialist has been notified.');
      setShowReschedule(false); setSlots([]);
    } catch (err: any) {
      setError(err?.message || 'The meeting could not be rescheduled.');
      await loadSlots(fromDate);
    } finally { setSaving(false); }
  };
  ...
  const cancel = async () => {
    setSaving(true); setError(''); setSuccess('');
    try {
      const updated = await bookingManagementService.cancel(token, reason);
      setBooking(updated);
      setSuccess('Your meeting has been cancelled and your ProFox specialist has been notified.');
      setShowCancel(false);
    } catch (err: any) { setError(err?.message || 'The meeting could not be cancelled.'); }
    finally { setSaving(false); }
  };
  ...
  const activeMeeting = booking.status === 'Confirmed' && ['Scheduled', 'Rescheduled'].includes(booking.meetingStatus);
  const attendanceConfirmed = booking.attendanceStatus === 'Confirmed';

  return <div className="min-h-screen bg-slate-50 text-slate-900"><header className="border-b border-slate-200 bg-white"><div className="mx-auto max-w-5xl px-5 py-8 sm:px-8"><Link to="/book-a-meeting" className="inline-flex items-center gap-2 text-xs font-black text-slate-500 hover:text-[#000080]"><ArrowLeft className="h-4 w-4" /> Booking page</Link><div className="mt-5 text-[10px] font-black uppercase tracking-[0.18em] text-[#FF0E0E]">Secure meeting management</div><h1 className="mt-2 text-3xl font-black">{booking.companyName}</h1><p className="mt-2 text-sm text-slate-500">Reference {booking.bookingReference}</p></div></header><main className="mx-auto max-w-5xl px-5 py-8 sm:px-8">
    ...
    <section className="grid gap-5 lg:grid-cols-[1fr_320px]"><div className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm sm:p-8"><div className="flex items-start justify-between gap-4"><div><div className="text-xs font-black uppercase tracking-wider text-slate-400">{booking.meetingType}</div><h2 className="mt-2 text-2xl font-black">{prettyFull(booking.startAt, visitorTimezone)}</h2><p className="mt-2 text-sm text-slate-500">Shown in your timezone: {visitorTimezone}</p></div><span className={`rounded-full px-3 py-1.5 text-xs font-black ${booking.status === 'Confirmed' ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{booking.status}</span></div>
      <div className="mt-6 grid gap-3 sm:grid-cols-2"><Info label="Specialist" value={booking.expertName} /><Info label="Service" value={booking.serviceInterest || 'ProFox consultation'} /><Info label="Duration" value={`${Math.max(1, Math.round((new Date(booking.endAt).getTime() - new Date(booking.startAt).getTime()) / 60000))} minutes`} /><Info label="Attendance" value={attendanceConfirmed ? 'Confirmed' : activeMeeting ? 'Awaiting confirmation' : booking.meetingStatus} /></div>
      <div className="mt-7 flex flex-wrap gap-3">
        {activeMeeting && booking.meetingUrl && <a href={booking.meetingUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-2xl bg-[#FF0E0E] px-4 py-2.5 text-xs font-black text-white"><ExternalLink className="h-4 w-4" /> Join meeting</a>}
        {activeMeeting && !booking.meetingUrl && <button type="button" onClick={()=>void load()} className="inline-flex items-center gap-2 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-black text-[#000080]"><RefreshCw className="h-4 w-4" /> Refresh meeting link</button>}
        {activeMeeting && !attendanceConfirmed && <button type="button" disabled={saving} onClick={()=>void confirmAttendance()} className="inline-flex items-center gap-2 rounded-2xl bg-[#000080] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50"><CheckCircle2 className="h-4 w-4" /> Yes, I am attending</button>}
        <button type="button" onClick={() => downloadIcs({ title: `ProFox - ${booking.serviceInterest || booking.meetingType}`, description: `Meeting with ${booking.expertName}. Booking ${booking.bookingReference}`, startAt: booking.startAt, endAt: booking.endAt, url: booking.meetingUrl || window.location.href, uid: `${booking.bookingReference}@profoxwebdesigner.com` }, `${booking.bookingReference}.ics`)} className="inline-flex items-center gap-2 rounded-2xl border border-slate-200 px-4 py-2.5 text-xs font-black text-slate-700 hover:bg-slate-50"><CalendarDays className="h-4 w-4" /> Add to calendar</button>
        {booking.canReschedule && <button type="button" onClick={()=>{setShowReschedule(true);setShowCancel(false);void loadSlots();}} className="inline-flex items-center gap-2 rounded-2xl border border-[#000080]/20 px-4 py-2.5 text-xs font-black text-[#000080]"><RefreshCw className="h-4 w-4" /> Reschedule</button>}
        {booking.canCancel && <button type="button" onClick={()=>{setShowCancel(true);setShowReschedule(false);}} className="inline-flex items-center gap-2 rounded-2xl border border-red-200 px-4 py-2.5 text-xs font-black text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /> Cancel</button>}
      </div>
      {activeMeeting && !booking.meetingUrl && <div className="mt-5 rounded-2xl border border-blue-100 bg-blue-50/60 p-4 text-xs leading-5 text-blue-900">Your joining link is still being prepared. This page always shows the latest meeting link. You do not need to search through old emails.</div>}
    </div><aside className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center gap-3">{booking.expertAvatarUrl ? <img src={booking.expertAvatarUrl} alt="" className="h-12 w-12 rounded-2xl object-cover" /> : <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-slate-100"><UserRound className="h-5 w-5 text-slate-400" /></div>}<div><div className="font-black">{booking.expertName}</div><div className="mt-1 text-xs text-[#000080]">{booking.expertHeadline || 'ProFox Specialist'}</div></div></div><p className="mt-5 text-xs leading-6 text-slate-500">{booking.expertCountry ? `${booking.expertCountry} · ` : ''}Your meeting stays connected to the same ProFox CRM history when you reschedule. No duplicate customer record is created.</p><a href="https://www.profoxwebdesigner.com/" className="mt-5 inline-flex text-xs font-black text-[#000080]">ProFox · From site to system.</a></aside></section>
    ...
    {booking.status === 'Cancelled' && <section className="mt-6 rounded-3xl border border-slate-200 bg-white p-7 text-center shadow-sm"><CheckCircle2 className="mx-auto h-8 w-8 text-slate-300" /><h2 className="mt-4 text-xl font-black">This meeting is cancelled</h2><p className="mt-2 text-sm text-slate-500">If you would still like to speak with us, choose a fresh time with any available specialist.</p><Link to="/book-a-meeting" className="mt-5 inline-flex rounded-2xl bg-[#000080] px-5 py-3 text-sm font-black text-white">Book another meeting</Link></section>}
  </main></div>;
}
