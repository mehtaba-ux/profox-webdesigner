...
export default function PublicBookingFlow({ embedded = false, topContent, onRequestQuote }: {
  ...
  if (!settings?.active) return <div className={`${shellClass} text-center`}><CalendarDays className="mx-auto h-9 w-9 text-slate-300" /><h2 className="pf-card-title mt-4 text-slate-950">Booking is currently unavailable</h2><p className="mt-2 text-sm text-slate-500">Send us an enquiry and our team will help you with the next step.</p>{onRequestQuote && <button type="button" onClick={onRequestQuote} className={`mt-5 bg-[var(--brand-primary)] px-5 py-2.5 text-xs font-bold text-white ${buttonRadius}`}>Get a quote instead</button>}</div>;
      ...
      <AnimatePresence mode="wait">
        <motion.div key={step} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.22, ease: 'easeOut' }}>
          {step === 1 && <section className="mt-4">
            ...
            {experts.length === 0 ? <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-7 text-center"><UsersRound className="mx-auto h-9 w-9 text-slate-300" /><h3 className="pf-card-title mt-3 text-slate-950">Our specialist calendar is being prepared</h3><p className="mt-2 text-xs leading-5 text-slate-500">You can still send your project details and our team will respond with the right next step.</p>{onRequestQuote && <button type="button" onClick={onRequestQuote} className={`mt-4 bg-[var(--brand-primary)] px-4 py-2.5 text-xs font-bold text-white ${buttonRadius}`}>Get a quote</button>}</div> : filteredExperts.length === 0 ? <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-6 text-center text-xs text-slate-500">No specialist matches these filters. Clear a filter or choose another service.</div> : <div className="mt-5 grid gap-3 sm:grid-cols-2 2xl:grid-cols-3">{filteredExperts.map(expert => <button type="button" key={expert.salespersonId} onClick={() => void chooseExpert(expert)} className="group rounded-2xl border border-slate-200 bg-white p-4 text-left transition-all hover:-translate-y-0.5 hover:border-[var(--brand-primary)]/20 hover:shadow-[0_12px_32px_rgba(15,23,42,0.07)]">
          ...
          {step === 3 && selectedExpert && selectedSlot && <section className="mt-4">
            ...
            <form onSubmit={submitBooking} className="mt-4">
              ...
              <button type="submit" disabled={submitting} className={`mt-4 inline-flex min-h-10 w-full items-center justify-center gap-2 bg-[var(--brand-primary)] px-5 text-xs font-bold text-white shadow-[0_12px_30px_rgba(0,0,128,0.16)] transition-all hover:-translate-y-0.5 hover:bg-[var(--brand-primary-hover)] disabled:opacity-60 sm:w-auto ${buttonRadius}`}>{submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <CalendarDays className="h-4 w-4" />}{submitting ? 'Confirming securely…' : 'Confirm meeting'}</button>
