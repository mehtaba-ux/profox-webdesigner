...
export default function ClientRelationshipHistory() {
  ...
  const load = async (quiet = false) => {
    ...
    const { data: result, error: rpcError } = await supabase.rpc('client_get_relationship_history');
  ...
  const sendReply = async () => {
    ...
    setSending(true);
    setError('');
    const { error: sendError } = await supabase.rpc('client_portal_send_conversation_message', {
