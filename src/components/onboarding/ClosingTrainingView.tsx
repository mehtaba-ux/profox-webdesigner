...
import ClosingTraining from'./ClosingTraining';

interface ClosingTrainingViewProps{onComplete?:()=>void}

export default function ClosingTrainingView(_props:ClosingTrainingViewProps){
 const{user}=useAuth();
 ...
 const load=async()=>{
  ...
  try{
   ...
   const target=(modulesResult.data||[]).find(item=>item.slug==='closing'||item.slug==='closing-training');
   if(!target)throw new Error('Closing Training module is unavailable.');
  ...
  }catch(e:any){setError(e?.message||'Module 14 could not be loaded.');}
 ...
 if(error||!module)return <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error||'Closing Training module is unavailable.'}</div>;
 return <ClosingTraining module={module} lessons={lessons} progress={progress} onRefresh={load}/>;
}
