...
import ObjectionHandlingTraining from'./ObjectionHandlingTraining';

interface ObjectionHandlingViewProps{onComplete?:()=>void}

export default function ObjectionHandlingView(_props:ObjectionHandlingViewProps){
 const{user}=useAuth();
 ...
 const load=async()=>{
  ...
  try{
   ...
   const target=(modulesResult.data||[]).find(item=>item.slug==='objections'||item.slug==='objection-handling');
   if(!target)throw new Error('Objection Handling module is unavailable.');
  ...
  }catch(e:any){setError(e?.message||'Module 13 could not be loaded.');}
 ...
 if(error||!module)return <div className="rounded-2xl border border-red-200 bg-red-50 p-4 text-sm text-red-700">{error||'Objection Handling module is unavailable.'}</div>;
 return <ObjectionHandlingTraining module={module} lessons={lessons} progress={progress} onRefresh={load}/>;
}
