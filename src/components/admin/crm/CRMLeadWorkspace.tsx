...
function ConnectButtons({lead,onMeeting}:{lead:CRMLead;onOpen:(lead:CRMLead,tab:LeadDrawerTab)=>void;onMeeting:()=>void}){return <div className="flex items-center justify-end gap-1.5"><button onClick={event=>{event.stopPropagation();window.location.assign(`/admin/app/crm?tab=inbox&lead=${encodeURIComponent(lead.id)}`);}} title="Open unified customer conversation" className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2.5 text-slate-600 hover:border-[#000080]/30 hover:bg-blue-50 hover:text-[#000080]"><MessageCircle className="h-4 w-4"/><Mail className="h-3.5 w-3.5"/><span className="hidden text-[10px] font-black xl:inline">Conversation</span></button><button onClick={event=>{event.stopPropagation();onMeeting();}} title="Schedule meeting" className="rounded-xl bg-[#000080] p-2.5 text-white hover:bg-[#000066]"><CalendarDays className="h-4 w-4"/></button></div>}
