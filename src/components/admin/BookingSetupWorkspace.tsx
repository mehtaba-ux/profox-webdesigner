...
export default function BookingSetupWorkspace() {
  ...
  const loadTarget = async (id: string, person: UserProfile) => {
    ...
    try {
      ...
      setBookingProfile(storedProfile || profileDefaults(person,meetingSettings));
  ...
  const saveProfile = async () => {
    ...
    try {
      ...
      const saved = await bookingService.saveProfile(bookingProfile);
