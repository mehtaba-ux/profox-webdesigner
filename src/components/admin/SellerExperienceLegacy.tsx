...
function MeetingActions({ meeting, navigate }: { meeting: SellerEnhancedMeeting; navigate: ReturnType<typeof useNavigate> }) {
  return <article className="rounded-2xl border border-slate-200 bg-slate-50 p-4"><div className="text-sm font-black">{meeting.title || meeting.attendeeName || 'Sales Meeting'}</div><div className="mt-1 text-xs text-slate-500">{shortDateTime(meeting.startAt, meeting.timezone)}</div><div className="mt-3 flex flex-wrap gap-2"><button onClick={() => navigate(meeting.prepUrl)} className="rounded-lg bg-[#000080] px-3 py-2 text-[10px] font-black text-white">Prepare Meeting</button>{meeting.crmFocusUrl && <button onClick={() => navigate(meeting.crmFocusUrl!)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-black">Open CRM</button>}{meeting.meetingUrl && <a href={meeting.meetingUrl} target="_blank" rel="noreferrer" className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-black">Join Meeting</a>}<button onClick={() => navigate(meeting.manageUrl)} className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-[10px] font-black">Reschedule / Manage</button></div></article>;
}
