...
export default function SalesAccountSetup({ onCompleted }: { onCompleted?: () => Promise<void> | void }) {
  ...
  const connectGoogle = async () => {
    ...
    try {
      ...
      const authorizationUrl = await googleCalendarService.startConnection(
        snapshot.connection.status === 'reconnect_required',
        '/admin/workspace',
      );
  ...
  const completeTourStep = async () => {
    ...
    if (reviewStep !== expected) {
      setReviewStep(Math.min(6, expected));
      return;
    }
    setBusy(true);
    setError('');
    ...
    <div className="mx-auto max-w-6xl space-y-6 py-2 sm:py-4">
        ...
        <div className="bg-gradient-to-br from-[#000080] to-[#1111a8] px-5 py-7 text-white sm:px-8 sm:py-9">
          <div className="flex flex-col gap-6 lg:flex-row lg:items-end lg:justify-between">
            <div className="max-w-3xl">
              ...
              <p className="mt-3 max-w-2xl text-sm leading-6 text-blue-100">Complete these steps once so your profile looks professional, customer meetings work correctly and you know exactly how ProFox CRM should be used. Your progress is saved automatically.</p>
            </div>
      ...
      <section className={`rounded-[28px] border bg-white p-5 shadow-sm sm:p-7 ${technicalReady ? 'border-slate-200' : 'border-amber-200'}`}>
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1"><div className="text-[10px] font-black uppercase tracking-[0.16em] text-violet-700">Part 2 · CRM guided tour</div><h2 className="mt-1 text-xl font-black">Learn the ProFox sales workflow</h2><p className="mt-1 text-sm leading-6 text-slate-500">Complete the six short steps in order. You can always move back to revise an earlier step.</p></div>
        </div>
