...
export default function ContentRecruitmentDashboard(){
  ...
  const load=async()=>{
    ...
    try{
      ...
      if(!jobRow)throw new Error('The Content Writer job configuration is unavailable.');
    ...
    }catch(err:any){setError(err?.message||'Content Writer recruitment could not be loaded.');}
...
function CandidateDetail({applicant,snapshot,policy,latestAssessment,interviews,linkedProfile,portfolioTask,busy,canAdvance,isAdmin,onRefresh,onAdvance,onIssueAgreement,onSendAccess,onApproveFinal,onActivate,onAssess,onSchedule,onCompleteInterview,onClose}:any){
  ...
  const rubric=Array.isArray(policy?.rubric)?policy.rubric:[];
  const total=rubric.reduce((sum:number,item:any)=>sum+Math.max(0,Number(item.maxPoints||0)),0);
  const earned=rubric.reduce((sum:number,item:any)=>sum+Math.min(Number(item.maxPoints||0),Math.max(0,Number(scores[item.key]??0))),0);
  const scoredCriteriaCount=rubric.filter((item:any)=>Object.prototype.hasOwnProperty.call(scores,item.key)&&Number.isFinite(Number(scores[item.key]))).length;
  const rubricComplete=rubric.length>0&&scoredCriteriaCount===rubric.length;
  ...
  return <section className="space-y-5">
    ...
    {policy?.assessmentRequired&&<div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><div className="text-[10px] font-black uppercase text-[#000080]">Structured assessment</div><h3 className="mt-1 text-sm font-black">{stageLabel(String(applicant.stage))}</h3></div>{latestAssessment&&<StageBadge stage={`${latestAssessment.status} · ${latestAssessment.score}%`} closed={latestAssessment.status==='Failed'}/>}</div><div className="mt-5 grid gap-3 sm:grid-cols-2">{rubric.map((item:any)=><label key={item.key} className="rounded-xl border border-slate-200 p-3 text-xs font-bold text-slate-700">{item.label}<span className="ml-1 text-slate-400">/ {item.maxPoints}</span><input type="number" min={0} max={item.maxPoints} value={scores[item.key]??''} onChange={e=>{const raw=e.target.value;setScores(s=>{const next={...s};if(raw==='')delete next[item.key];else next[item.key]=Number(raw);return next})}} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2"/></label>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><input value={evidenceUrl} onChange={e=>setEvidenceUrl(e.target.value)} placeholder="Evidence / work URL" className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs"/><input value={evidence} onChange={e=>setEvidence(e.target.value)} placeholder="Evidence summary" className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs"/></div><textarea value={assessmentNotes} onChange={e=>setAssessmentNotes(e.target.value)} rows={3} placeholder="Reviewer notes (required for retry or fail)" className="mt-3 w-full rounded-xl border border-slate-200 p-3 text-xs"/><div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs font-black text-slate-600">Calculated score: {score}% · pass {policy.passingScore??0}% · {scoredCriteriaCount}/{rubric.length} criteria scored</span><div className="flex gap-2">{applicant.stage==='Portfolio Review'&&isFinalPortfolioAttempt?<button disabled={busy||!rubricComplete||assessmentNotes.trim().length<10} onClick={()=>review('Failed')} className="rounded-xl border border-red-200 bg-white px-4 py-2.5 text-xs font-black text-red-700 disabled:opacity-40">Fail Portfolio Review</button>:<button disabled={busy||!rubricComplete||assessmentNotes.trim().length<10} onClick={()=>review('Retry Required')} className="rounded-xl border border-amber-200 bg-white px-4 py-2.5 text-xs font-black text-amber-700 disabled:opacity-40">{applicant.stage==='Portfolio Review'?'Request Revision':'Require Retry'}</button>}<button disabled={busy||!rubricComplete||score<(policy.passingScore??0)||(applicant.stage==='Portfolio Review'&&!portfolioCases.length)} onClick={()=>review('Passed')} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">Record Pass</button></div></div></div>}
    ...
    <div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><h3 className="text-sm font-black">Next protected action</h3><div className="mt-4 flex flex-wrap gap-2">
      ...
      {applicant.stage==='Activated'&&<div className="rounded-xl bg-emerald-50 px-4 py-3 text-xs font-semibold text-emerald-800"><CheckCircle2 className="mr-1 inline h-4 w-4"/>Active Content Writer. Eligible for approved project assignment with independent quality review.</div>}
    </div></div>
...
function SecureAsset({applicantId,assetType,label}:{applicantId:string;assetType:'cv'|'video';label:string}){const [loading,setLoading]=useState(false);const [error,setError]=useState('');const open=async()=>{setLoading(true);setError('');try{const{data,error:invokeError}=await supabase.functions.invoke('recruitment-private-asset-url',{body:{applicantId,assetType}});if(invokeError)throw invokeError;if(!data?.signedUrl)throw new Error(data?.error||'Secure recruitment asset link was not returned.');window.open(data.signedUrl,'_blank','noopener,noreferrer')}catch(err:any){setError(err?.message||'Secure recruitment asset could not be opened.')}finally{setLoading(false)}};return <div><button disabled={loading} onClick={()=>void open()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-4 py-2.5 text-xs font-black text-slate-700 disabled:opacity-50">{loading?<Loader2 className="h-4 w-4 animate-spin"/>:<BriefcaseBusiness className="h-4 w-4"/>}{label}</button>{error&&<div className="mt-1 max-w-xs text-[10px] font-semibold text-red-600">{error}</div>}</div>}
