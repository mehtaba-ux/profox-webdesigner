...
type RecruitmentTask = {
  id:string; stage:string; attemptNo:number; status:string; title:string; requiredItems:number; dueAt?:string;
...
function CandidateDetail({applicant,snapshot,policy,latestAssessment,interviews,linkedProfile,portfolioTask,busy,canAdvance,isAdmin,onRefresh,onAdvance,onIssueAgreement,onSendAccess,onApproveFinal,onActivate,onAssess,onSchedule,onCompleteInterview,onClose}:any){
  ...
  const review=(status:'Passed'|'Retry Required')=>{
    if(status==='Retry Required'&&assessmentNotes.trim().length<10)return;
    void onAssess({status,score:status==='Passed'?score:Math.min(score,Math.max(0,(policy?.passingScore||1)-1)),rubricScores:scores,criticalFailures:[],evidence,evidenceUrl,notes:assessmentNotes});
  };
  ...
  return <section className="space-y-5">
    ...
    {portfolioTask&&<div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-[10px] font-black uppercase tracking-[.14em] text-[#000080]">Secure portfolio evidence</div><h3 className="mt-1 text-sm font-black">{portfolioTask.title||'Three-case-study Portfolio Review'}</h3></div><StageBadge stage={`${portfolioTask.status} · Attempt ${portfolioTask.attemptNo||1}`}/></div>{portfolioTask.retryFeedback&&<div className="mt-4 rounded-xl bg-amber-50 p-3 text-xs text-amber-800"><strong>Retry feedback:</strong> {portfolioTask.retryFeedback}</div>}{portfolioCases.length?<div className="mt-4 grid gap-4">{portfolioCases.map((item,index)=><div key={index} className="rounded-2xl border border-slate-200 p-4"><div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between"><div><div className="text-[9px] font-black uppercase text-slate-400">Case study {index+1}</div><div className="mt-1 text-sm font-black text-slate-900">{item.projectTitle||'Untitled project'}</div><div className="mt-1 text-[11px] text-slate-500">{item.industry||'Industry not supplied'} · {item.contentType||'Content type not supplied'}</div></div>{item.sampleUrl&&<a href={item.sampleUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-xs font-black text-[#000080]">Open sample <ExternalLink className="h-3.5 w-3.5"/></a>}</div><div className="mt-4 grid gap-3 lg:grid-cols-2"><Evidence label="Brief / problem" value={item.briefProblem}/><Evidence label="Candidate contribution" value={item.contribution}/><Evidence label="Research process" value={item.researchProcess}/><Evidence label="Result / impact" value={item.resultOutcome}/></div></div>)}</div>:<div className="mt-4 rounded-2xl border border-dashed border-slate-200 p-5 text-xs text-slate-400">Portfolio task status: {portfolioTask.status}. Submitted case studies will appear here for scoring.</div>}</div>}

    {policy?.assessmentRequired&&<div className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><div><div className="text-[10px] font-black uppercase text-[#000080]">Structured assessment</div><h3 className="mt-1 text-sm font-black">{stageLabel(String(applicant.stage))}</h3></div>{latestAssessment&&<StageBadge stage={`${latestAssessment.status} · ${latestAssessment.score}%`} closed={latestAssessment.status==='Failed'}/>}</div><div className="mt-5 grid gap-3 sm:grid-cols-2">{(policy.rubric||[]).map((item:any)=><label key={item.key} className="rounded-xl border border-slate-200 p-3 text-xs font-bold text-slate-700">{item.label}<span className="ml-1 text-slate-400">/ {item.maxPoints}</span><input type="number" min={0} max={item.maxPoints} value={scores[item.key]??''} onChange={e=>setScores(s=>({...s,[item.key]:Number(e.target.value)}))} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2"/></label>)}</div><div className="mt-4 grid gap-3 sm:grid-cols-2"><input value={evidenceUrl} onChange={e=>setEvidenceUrl(e.target.value)} placeholder="Evidence / work URL" className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs"/><input value={evidence} onChange={e=>setEvidence(e.target.value)} placeholder="Evidence summary" className="rounded-xl border border-slate-200 px-3 py-2.5 text-xs"/></div><textarea value={assessmentNotes} onChange={e=>setAssessmentNotes(e.target.value)} rows={3} placeholder="Reviewer notes (required for retry)" className="mt-3 w-full rounded-xl border border-slate-200 p-3 text-xs"/><div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between"><span className="text-xs font-black text-slate-600">Calculated score: {score}% · pass {policy.passingScore??0}%</span><div className="flex gap-2"><button disabled={busy||assessmentNotes.trim().length<10} onClick={()=>review('Retry Required')} className="rounded-xl border border-amber-200 bg-white px-4 py-2.5 text-xs font-black text-amber-700 disabled:opacity-40">Require Retry</button><button disabled={busy||score<(policy.passingScore??0)||(applicant.stage==='Portfolio Review'&&!portfolioCases.length)} onClick={()=>review('Passed')} className="rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black text-white disabled:opacity-40">Record Pass</button></div></div></div>}
