...
export function lineTimelineText(line: Pick<CpqLine, 'itemType' | 'durationMinSnapshot' | 'durationMaxSnapshot' | 'durationUnitSnapshot' | 'timelineImpactSnapshot'>) {
  ...
  if (line.itemType === 'care_plan' && impact === 'parallel' && min == null && max == null) return 'Ongoing · no delivery extension';
...
export default function QuotationLineTimeline({ line, locked, isAdmin, onChange }: Props) {
  ...
  const impact = line.timelineImpactSnapshot || 'assessment_required';
  const isCustomScope = line.lineType === 'custom' && !line.salesProductId;
  ...
  return <div className="min-w-[170px]">
    {status === 'missing' ? <div className="rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-2">
      <p className="flex items-center gap-1.5 font-bold text-amber-800"><TriangleAlert className="h-3.5 w-3.5" />{isCustomScope ? 'Project timeline required' : 'Timeline required'}</p>
      <p className="mt-1 text-[10px] leading-4 text-amber-700">{isCustomScope ? 'Set the custom project duration before requesting quotation approval.' : 'Add a quotation-only estimate before this scope can be sent.'}</p>
    </div> : <div>
    ...
    {!editing && canEdit && <div className="mt-2 flex flex-wrap gap-1.5">
      <button type="button" onClick={beginEdit} className="rounded-md border border-slate-200 px-2 py-1 text-[10px] font-bold text-[#000080] hover:bg-slate-50">
        {isCustomScope ? (status === 'missing' ? 'Set project timeline' : 'Edit project timeline') : status === 'missing' ? 'Add estimate' : hasProtectedCatalogTimeline && isAdmin ? 'Override' : 'Add / edit estimate'}
      </button>
    ...
    {editing && <div className="mt-2 rounded-xl border border-slate-200 bg-slate-50 p-2.5 text-[10px]">
      <div className="mb-2 flex items-center justify-between"><b className="text-slate-700">{isCustomScope ? 'Custom project timeline' : 'Quotation timeline'}</b><button type="button" onClick={() => setEditing(false)} className="text-slate-400">Cancel</button></div>
      {isCustomScope && <p className="mb-2 leading-4 text-slate-500">This estimate is used by the quotation to calculate the customer-facing overall delivery timeline.</p>}
      ...
      <label className="mt-2 block"><span className="mb-1 block text-slate-500">Timeline impact</span><select value={editorImpact} onChange={e => setEditorImpact(e.target.value)} className="w-full rounded-md border border-slate-200 bg-white px-2 py-1.5"><option value="base">Base project timeline</option><option value="additive">Adds to project timeline</option><option value="parallel">Runs in parallel</option><option value="assessment_required">Assessment Required</option></select></label>
      ...
      <div className="mt-2 flex justify-end"><button type="button" onClick={saveEstimate} className="rounded-md bg-[#000080] px-2.5 py-1.5 font-bold text-white">Save project timeline</button></div>
    </div>}
  </div>;
}