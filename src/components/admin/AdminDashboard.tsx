...
export default function AdminDashboard() {
  ...
  const bgMain = isDarkMode ? 'bg-[#080915] text-slate-100' : 'bg-slate-50 text-slate-800';
    ...
    return (
      <div className={`min-h-screen ${bgMain} flex flex-col items-center justify-center p-6 font-sans transition-colors duration-200`}>
        ...
        <div className="absolute top-6 right-6">
          <button
            ...
            className={`p-2.5 rounded-full border transition-all flex items-center justify-center shadow-sm ${
              isDarkMode 
                ? 'bg-[#1a1c36] border-slate-200 text-amber-400 hover:text-amber-300' 
        ...
        <div className={`max-w-md w-full ${cardBg} rounded-2xl p-8 shadow-xl relative overflow-hidden transition-all duration-200`}>
          ...
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#000080] via-[#FF0E0E] to-[#000080]" />

          {/* Header & Logo */}
          <div className="text-center mb-8">
            <div className={`inline-flex items-center justify-center w-12 h-12 rounded-xl mb-4 ${
              isDarkMode ? 'bg-[#FF0E0E]/10 text-[#FF0E0E]' : 'bg-[#000080]/5 text-[#000080]'
            }`}>
              <Lock className="w-5 h-5" />
            </div>
            <h1 className={`text-2xl font-bold tracking-tight ${textHeading}`}>ProFox CMS Studio</h1>
            <p className={`text-xs mt-1 ${textMuted}`}>Sign in with your enterprise credentials to manage content and visual themes</p>
          </div>
          ...
          <div className="relative my-6">
            ...
            <div className="relative flex justify-center text-[10px] font-bold uppercase">
              <span className={`${isDarkMode ? 'bg-[#101226]' : 'bg-white'} px-3 ${textMuted}`}>Or continue with</span>
            </div>
          </div>
...
{/* TAB MEDIA: MEDIA LIBRARY */}
          ...
          {activeTab === 'inbox' && role === 'sales_team' && (
            <SalesChatInbox />
          )}
