...
export default function TalentPartnerAdmin() {
  ...
  const uniqueVisitors = Number(data.trackingMetrics?.uniqueVisitors || 0);
...
function Overview({data,visits,activePartners,uniqueVisitors,pendingRewards,paidRewards}:{data:TalentPartnerAdminData;visits:any[];activePartners:number;uniqueVisitors:number;pendingRewards:number;paidRewards:number}) {
  const applications=Number(data.trackingMetrics?.attributedApplications || 0);
  const activated=Number(data.trackingMetrics?.activatedHires || 0);
  return <div className="space-y-6">
    {data.readiness?.automaticExternalSettlementConfigured===false&&<Alert>External PayPal, Wise, or bank transfer remains a controlled manual settlement. A second active Admin must confirm the transaction reference after the batch creator sends the payment.</Alert>}
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-6"><Kpi icon={Users} label="Partners" value={data.partners.length} detail={`${activePartners} active`}/><Kpi icon={MousePointerClick} label="Visits" value={Number(data.trackingMetrics?.visits||0)} detail={`${uniqueVisitors} unique`}/><Kpi icon={FileText} label="Attributed applications" value={applications}/><Kpi icon={UserCheck} label="Activated hires" value={activated}/><Kpi icon={CircleDollarSign} label="Pending rewards" value={moneyNumber(pendingRewards)}/><Kpi icon={WalletCards} label="Paid rewards" value={moneyNumber(paidRewards)}/></div>
...
function PlanEditor({job,existing,onSave}:{job:any;existing:any;onSave:any}) {
  const salesJob=job.application_type==='sales_representative'||job.slug==='independent-sales-representative';
  ...
  const [model,setModel]=useState(salesJob?'sales':existing?.reward_model||defaultModel);
...
function ProjectRewards({data,users,run}:{data:TalentPartnerAdminData;users:any[];run:any}) {
  ...
  return <Panel title="Approve a qualifying project" icon={BriefcaseBusiness}><p className="mb-5 text-sm leading-6 text-slate-500">Only fully completed projects assigned to the referred worker can count. Where a canonical worker earning exists, the database automatically uses that amount and currency. Otherwise, enter the approved payable amount and a clear accounting reference in Notes.</p><form onSubmit={submit} className="grid gap-4 lg:grid-cols-2"><Select label="Activated project-based referral" value={referralId} onChange={v=>{setReferralId(v);setProjectId('')}} options={referralOptions}/><Select label="Completed assigned project" value={projectId} onChange={setProjectId} options={projectOptions}/><Input label="Approved worker payment amount" type="number" value={amount} onChange={setAmount}/><Input label="Currency" value={currency} onChange={setCurrency}/><div className="lg:col-span-2"><label className="label">Notes / accounting reference</label><textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={3} className="input resize-y"/></div><div className="lg:col-span-2"><button disabled={!referralId||!projectId||Number(amount)<=0||notes.trim().length<10} className="btn-primary">Approve project reward</button></div></form></Panel>;
}
...
function Payouts({data,users,run}:{data:TalentPartnerAdminData;users:any[];run:any}) {
  ...
  const [adjustmentDrafts,setAdjustmentDrafts]=useState<Record<string,{resolution:string;reference:string}>>({});
  const name=(id:string)=>users.find(u=>u.id===id)?.full_name||'Partner';
  const inactiveWithApproved=data.partners.filter(p=>['Suspended','Closed'].includes(p.status)&&data.rewards.some(r=>r.partner_user_id===p.user_id&&r.status==='Approved'&&!r.payout_id));
  const openAdjustments=data.adjustments.filter(a=>a.status==='Open');
  return <div className="space-y-6">
    <Panel title="Payout controls" icon={WalletCards}><p className="text-sm leading-6 text-slate-500">Only approved, due rewards with verified payout details enter a single-currency batch. The Admin who creates a batch cannot confirm its external payment.</p><button onClick={()=>run(()=>talentPartnerService.adminCreatePayoutBatch(),'Payout batch created. A different Admin must confirm each transfer.')} className="btn-primary mt-4">Create payout batch</button></Panel>
    {openAdjustments.length>0&&<Panel title="Financial adjustments requiring resolution" icon={ShieldCheck}><div className="space-y-4">{openAdjustments.map(a=>{const draft=adjustmentDrafts[a.id]||{resolution:'',reference:''};return <div key={a.id} className="rounded-xl border border-amber-200 bg-amber-50 p-4"><div className="text-sm font-black text-amber-950">{name(a.partner_user_id)} · {money(a.amount,a.currency)}</div><p className="mt-1 text-xs leading-5 text-amber-800">{a.reason}</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><input className="input" placeholder="Resolution (minimum 10 characters)" value={draft.resolution} onChange={e=>setAdjustmentDrafts(v=>({...v,[a.id]:{...draft,resolution:e.target.value}}))}/><input className="input" placeholder="Accounting / recovery reference" value={draft.reference} onChange={e=>setAdjustmentDrafts(v=>({...v,[a.id]:{...draft,reference:e.target.value}}))}/></div><button disabled={draft.resolution.trim().length<10||draft.reference.trim().length<3} onClick={()=>run(()=>talentPartnerService.adminResolveAdjustment(a.id,draft.resolution,draft.reference),'Financial adjustment resolved with an audit reference.')} className="btn-primary mt-3">Resolve adjustment</button></div>})}</div></Panel>}
    {inactiveWithApproved.length>0&&<Panel title="Inactive partner final settlement" icon={BadgeDollarSign}><p className="mb-4 text-xs leading-5 text-slate-500">Approved obligations remain payable after suspension or closure when a preserved verified payout profile exists.</p><div className="space-y-3">{inactiveWithApproved.map(p=><div key={p.user_id} className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-slate-200 p-4"><div><div className="text-sm font-black">{name(p.user_id)}</div><div className="text-xs text-slate-500">{p.status}</div></div><button onClick={()=>run(()=>talentPartnerService.adminCreateFinalSettlement(p.user_id),'Final settlement batch created.')} className="btn-light">Create final settlement</button></div>)}</div></Panel>}
    <Panel title="Partner payouts"><div className="space-y-3">{data.payouts.map(p=><div key={p.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-col justify-between gap-3 lg:flex-row lg:items-center"><div><div className="text-sm font-black text-slate-900">{name(p.partner_user_id)} · {money(p.amount,p.currency)}</div><div className="mt-1 text-xs text-slate-500">{p.entry_count} reward entries · {p.payout_method_snapshot||'method not set'}</div></div><Status value={p.status}/></div>{p.status==='Ready'&&<div className="mt-4"><p className="mb-2 text-[10px] font-bold text-slate-500">A different active Admin from the batch creator must confirm this external transfer.</p><div className="flex flex-col gap-2 sm:flex-row"><input value={transactions[p.id]||''} onChange={e=>setTransactions(v=>({...v,[p.id]:e.target.value}))} placeholder="PayPal / Wise / bank transaction reference" className="input flex-1"/><button disabled={!transactions[p.id]?.trim()} onClick={()=>run(()=>talentPartnerService.adminMarkPayoutPaid(p.id,transactions[p.id]),'Payout marked paid with maker-checker validation.')} className="btn-primary">Confirm paid</button></div></div>}</div>)}{!data.payouts.length&&<Empty text="No Talent Partner payouts yet."/>}</div></Panel>
  </div>;
}
...
function Select({label,value,onChange,options,disabled=false}:{label:string;value:string;onChange:(v:string)=>void;options:readonly SelectOption[];disabled?:boolean}){return <label><span className="label">{label}</span><select value={value} disabled={disabled} onChange={e=>onChange(e.target.value)} className="input disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-500">{options.map(([v,l],index)=><option key={`${v}-${index}`} value={v}>{l}</option>)}</select></label>}
