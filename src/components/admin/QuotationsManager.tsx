...
export default function QuotationsManager({ onNavigate }: { onNavigate?: (tab: string, metadata?: any) => void }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (requestedStatus !== 'Draft' && !editingQuotation.opportunityId) {
      setError('Link this quotation to its CRM opportunity before requesting approval or sending it to the customer.');
      return;
    }
  ...
  return <div className="space-y-6">
    ...
    {isModalOpen && editingQuotation && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm"><div className="flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl"><div className="flex items-center justify-between border-b border-slate-100 bg-slate-50 px-8 py-5"><div><h2 className="text-lg font-bold">{editingQuotation.id ? `Edit ${editingQuotation.quotationNumber}` : 'Create New Quotation'}</h2><p className="text-xs text-slate-500">Catalog pricing and timing are snapshotted server-side before customer delivery.</p></div><button onClick={() => setIsModalOpen(false)} className="rounded-full p-2 hover:bg-slate-200"><X className="h-5 w-5" /></button></div><div className="flex-1 overflow-y-auto"><form id="quotationForm" onSubmit={handleSave} className="space-y-8 p-8">
      <div className="grid gap-6 md:grid-cols-3"><div className="space-y-4"><h3 className="flex items-center gap-2 border-b border-blue-100 pb-2 text-xs font-black uppercase tracking-widest text-[#000080]"><User className="h-4 w-4" />Customer Info</h3><Field label="Customer / Company"><input required value={editingQuotation.customerName || ''} onChange={e => setEditingQuotation({ ...editingQuotation, customerName: e.target.value })} /></Field><Field label="Email"><input type="email" value={editingQuotation.email || ''} onChange={e => setEditingQuotation({ ...editingQuotation, email: e.target.value })} /></Field><Field label="CRM Opportunity"><div className="space-y-2"><select required={(editingQuotation.status || 'Draft') !== 'Draft'} value={editingQuotation.opportunityId || ''} disabled={editingQuotation.status !== 'Draft' && !isAdmin} onChange={e => { const opp = opportunities.find(o => o.id === e.target.value); setEditingQuotation({ ...editingQuotation, opportunityId: e.target.value || undefined, customerName: opp ? (opp.companyName || opp.name) : editingQuotation.customerName, email: opp?.email || editingQuotation.email, phone: opp?.phone || editingQuotation.phone, country: opp?.country || editingQuotation.country }); }}><option value="">Select opportunity</option>{opportunities.map(opp => <option key={opp.id} value={opp.id}>{opp.companyName || opp.name} ({opp.stage})</option>)}</select>{!editingQuotation.opportunityId && <p className="flex items-start gap-1.5 text-[10px] leading-4 text-amber-700"><AlertTriangle className="mt-0.5 h-3 w-3 shrink-0" />A Draft can be prepared without a link, but approval, sending and payment require the correct CRM opportunity.</p>}</div></Field></div>
