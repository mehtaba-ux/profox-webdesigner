...
export default function MeetingManageView() {
  ...
  const save = async () => {
    ...
    try {
      ...
      const updated = await meetingService.reschedule(meeting.id, startIso, endIso, timezone, meetingUrl || undefined);
      setMeeting(updated);
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-4xl p-4 sm:p-8">
      {loading ? <div className="flex min-h-80 items-center justify-center"><Loader2 className="h-8 w-8 animate-spin text-[#000080]" /></div> : error && !meeting ? <Notice type="error" text={error} /> : meeting ? <div className="space-y-5">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-start justify-between gap-4"><div><div className="text-[10px] font-black uppercase tracking-wider text-[#000080]">{meeting.meetingType}</div><h2 className="mt-1 text-2xl font-black">{meeting.title}</h2><p className="mt-2 text-sm text-slate-500">{meeting.attendeeName || meeting.attendeeEmail || 'CRM contact'}</p></div><span className="rounded-full bg-slate-100 px-3 py-1.5 text-[10px] font-black uppercase text-slate-600">{meeting.status}</span></div></section>
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm"><div className="mb-5 flex items-center gap-2"><CalendarDays className="h-5 w-5 text-[#000080]" /><h3 className="font-black">Reschedule</h3></div>{closed ? <div className="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-xs text-amber-900">This meeting is closed and can no longer be rescheduled.</div> : <div className="grid gap-4 md:grid-cols-2"><label className="text-xs font-bold text-slate-600">Start<input type="datetime-local" value={startLocal} onChange={e=>setStartLocal(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">Timezone<input value={timezone} onChange={e=>setTimezone(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold text-slate-600">Duration<select value={duration} onChange={e=>setDuration(Number(e.target.value))} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm">{settings.allowedDurations.map(value=><option key={value} value={value}>{value} minutes</option>)}</select></label><label className="text-xs font-bold text-slate-600">Meeting URL<input value={meetingUrl} onChange={e=>setMeetingUrl(e.target.value)} className="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm" /></label></div>}
