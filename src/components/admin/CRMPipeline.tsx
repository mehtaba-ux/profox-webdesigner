      ...
      <div className="flex-1 overflow-y-auto p-5 sm:p-7">
        ...
        {opportunity.saleActivation && <section className="mt-6 rounded-2xl border border-emerald-100 bg-emerald-50/30 p-4 shadow-sm">
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2">
            {opportunity.saleActivation.acceptedQuotation && <button type="button" onClick={() => navigate(opportunity.saleActivation!.acceptedQuotation!.url)} className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-black text-slate-700">Accepted quotation</button>}
            <button type="button" onClick={() => navigate(opportunity.saleActivation!.paymentWorkspaceUrl)} className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-black text-slate-700">{opportunity.saleActivation.canVerifyPayment && opportunity.saleActivation.operationalLabel === 'VERIFICATION REQUIRED' ? 'Review payment verification' : 'Open payments'}</button>
            {opportunity.saleActivation.nextAction?.kind === 'activity' && <button type="button" onClick={() => navigate(opportunity.saleActivation!.activityWorkspaceUrl)} className="min-h-10 rounded-xl border border-slate-200 bg-white px-3 text-[10px] font-black text-slate-700">Open Payment Follow-Up</button>}
          </div>
