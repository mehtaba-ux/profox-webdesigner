...
export default function PaymentsManager({ initialMetadata }: PaymentsManagerProps) {
  ...
  const fetchPayments = async () => {
    ...
    const { data, error: loadError } = await salesService.getPayments();
    if (loadError) setError(loadError.message || 'Failed to load payments.'); else setPayments(data || []);
    setLoading(false);
  };
