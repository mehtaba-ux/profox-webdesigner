...
export default function RecruitmentDashboard() {
  ...
  return <div className="space-y-6">
    ...
    {selectedDepartment === 'all' ? <RecruitmentOverview jobs={jobs} applicants={baseVisibleApplicants} onOpen={(department, jobId) => { setSelectedDepartment(department); void selectJob(jobId); }} /> : <>
      ...
      {view === 'board' ? <RecruitmentBoard applicants={scopedApplicants} stages={activeStageNames} onSelect={setSelectedApplicant} /> : <ApplicantList applicants={scopedApplicants} onSelect={setSelectedApplicant} />}
    </>}
...
function RecruitmentBoard({ applicants, stages, onSelect }: { applicants: ApplicantRecord[]; stages: string[]; onSelect: (applicant: ApplicantRecord) => void }) {
  ...
  return <div className="-mx-4 flex gap-4 overflow-x-auto px-4 pb-6 sm:-mx-6 sm:px-6">{stages.map(stage => { const rows = applicants.filter(applicant => applicant.stage === stage); return <div key={stage} className="w-[300px] min-w-[300px]"><div className="mb-2 flex items-center justify-between px-1"><h3 className="text-xs font-black uppercase tracking-wide text-slate-500">{stage}</h3><span className="rounded-full bg-slate-100 px-2 py-0.5 text-xs font-black text-slate-500">{rows.length}</span></div><div className="flex min-h-[360px] flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50/70 p-2">{rows.map(applicant => <CandidateCard key={applicant.id} applicant={applicant} onSelect={onSelect} />)}{!rows.length && <div className="flex flex-1 items-center justify-center text-sm font-bold text-slate-300">No candidates</div>}</div></div>; })}</div>;
}
