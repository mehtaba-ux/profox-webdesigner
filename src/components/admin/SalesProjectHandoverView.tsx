              ...
              <div className="space-y-7">
                ...
                <div className="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {isSeller && brief.canSubmit && <button type="button" onClick={() => void submit()} disabled={saving} className="inline-flex items-center gap-2 rounded-xl bg-[#000080] px-5 py-3 text-xs font-black text-white disabled:opacity-40 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2">{saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}{brief.lifecycleStatus === 'RETURNED_TO_SALES' || (brief.lifecycleStatus === 'ACCEPTED' && brief.sourceDrift) ? 'Resubmit to Delivery' : 'Send to Delivery'}</button>}
