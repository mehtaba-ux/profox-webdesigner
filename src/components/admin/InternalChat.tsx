...
export default function InternalChat() {
  ...
  const policySummary = 'Project-scoped access only. A conversation is available only while both participants are authorized on the same active project and their internal roles are permitted to connect. Removing an assignment or closing the project removes access automatically.';
  ...
  const filteredThreads = useMemo(() => {
    ...
    return threads.filter(thread =>
      `${thread.projectName} ${thread.otherFullName} ${thread.otherRole} ${thread.otherDepartment} ${thread.lastMessagePreview}`.toLowerCase().includes(q)
    );
  }, [threads, search]);
  ...
  const filteredContacts = useMemo(() => {
    ...
    return contacts.filter(contact => {
      const matches = !q || `${contact.projectName} ${contact.fullName} ${contact.role} ${contact.department}`.toLowerCase().includes(q);
      const alreadyOpen = threads.some(thread =>
        thread.projectId === contact.projectId && thread.otherUserId === contact.userId
      );
  ...
  const groupedContacts = useMemo(() => {
    ...
    filteredContacts.forEach(contact => {
      const key = contact.projectId;
      const existing = projects.get(key);
      if (existing) existing.items.push(contact);
      else projects.set(key, { projectId: key, projectName: contact.projectName, items: [contact] });
    });
    ...
    return (
      <div className="min-h-screen bg-slate-50 p-6 flex items-center justify-center">
        <div className="max-w-lg rounded-3xl border border-red-200 bg-white p-8 text-center shadow-sm">
          ...
          <p className="mt-2 text-sm leading-6 text-slate-500">Internal chat is available only to active ProFox staff accounts. Customer, client, pending and talent-partner accounts are excluded.</p>
    ...
    <div className="profox-app-shell min-h-screen bg-[#f3f7fc] text-slate-900">
        ...
        <div className="mx-auto flex max-w-[1500px] items-center justify-between gap-4 px-4 py-4 sm:px-6">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2"><h1 className="truncate text-base font-black">Internal Chat</h1><span className="rounded-full border border-emerald-200 bg-emerald-50 px-2 py-0.5 text-[9px] font-black uppercase tracking-wide text-emerald-700">Project Scoped</span></div>
              <p className="truncate text-[11px] text-slate-500">Assignment-controlled internal communication · no customer access</p>
            </div>
          </div>
      ...
      <main className="mx-auto max-w-[1500px] p-4 sm:p-6">
        <div className="mb-4 flex items-start gap-3 rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3">
          ...
          <div><div className="text-xs font-black text-slate-800">Project access enforced by Supabase</div><p className="mt-0.5 text-[11px] leading-5 text-slate-600">{policySummary} Legacy non-project threads are preserved but locked. Messages remain immutable in the application.</p></div>
        </div>
        ...
        <div className="grid min-h-[72vh] overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm lg:grid-cols-[380px_1fr]">
          <aside className={`${selectedThreadId ? 'hidden lg:flex' : 'flex'} min-h-[72vh] flex-col border-b border-slate-200 bg-slate-50/60 lg:border-b-0 lg:border-r`}>
            <div className="border-b border-slate-200 bg-white p-4">
              <div className="relative"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-400" /><input value={search} onChange={event => setSearch(event.target.value)} placeholder="Search projects, people or chats..." className="w-full rounded-xl border border-slate-200 bg-slate-50 py-2.5 pl-9 pr-3 text-xs outline-none focus:border-[#000080]" /></div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-3">
              ...
              {groupedContacts.map(group => <div key={group.projectId} className="mb-5"><div className="mb-2 flex items-center gap-2 px-2 text-[9px] font-black uppercase tracking-[0.16em] text-slate-400"><UsersRound className="h-3.5 w-3.5" /><span className="truncate">{group.projectName}</span></div><div className="space-y-1">{group.items.map(contact => <button key={`${contact.projectId}:${contact.userId}`} type="button" onClick={() => void startConversation(contact)} className="flex w-full items-center gap-3 rounded-2xl p-3 text-left transition hover:bg-white">
