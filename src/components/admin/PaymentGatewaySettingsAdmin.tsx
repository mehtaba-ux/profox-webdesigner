  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-8">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <StatusCard title="Automatic settlement" text="Verified gateway capture reuses Won → Client → Commission → Project." ok />
      </section>
      ...
      <ProviderCard title="Razorpay" icon={<CreditCard className="h-5 w-5" />} state={status?.razorpay} webhookUrl={razorpayWebhook} configuredText="Key ID + API Secret" webhookText="Razorpay webhook secret">
        ...
        <ProviderActions enabled={razorpay.enabled} setEnabled={value => setRazorpay({ ...razorpay, enabled: value })} busy={busy} provider="razorpay" onSave={saveProvider} onTest={testProvider} canTest={!!status?.razorpay?.configured} />
      </ProviderCard>
      ...
      <ProviderCard title="PayPal" icon={<WalletCards className="h-5 w-5" />} state={status?.paypal} webhookUrl={paypalWebhook} configuredText="Client ID + Client Secret" webhookText="PayPal Webhook ID">
        ...
        <ProviderActions enabled={paypal.enabled} setEnabled={value => setPaypal({ ...paypal, enabled: value })} busy={busy} provider="paypal" onSave={saveProvider} onTest={testProvider} canTest={!!status?.paypal?.configured} />
      </ProviderCard>
...
function ProviderCard({ title, icon, state, webhookUrl, configuredText, webhookText, children }: { title: string; icon: React.ReactNode; state?: GatewayProviderStatus; webhookUrl: string; configuredText: string; webhookText: string; children: React.ReactNode }) {
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-3"><div className="rounded-2xl bg-slate-100 p-3 text-[#000080]">{icon}</div><div><h2 className="font-black">{title}</h2><div className="mt-1 flex flex-wrap gap-2"><Badge ok={!!state?.configured}>{configuredText}: {state?.configured ? 'configured' : 'not configured'}</Badge><Badge ok={!!state?.webhookConfigured}>{webhookText}: {state?.webhookConfigured ? 'configured' : 'not configured'}</Badge><Badge ok={!!state?.enabled}>{state?.enabled ? 'Enabled' : 'Disabled'}</Badge></div></div></div>{state?.lastTestAt && <div className="text-right text-[10px] text-slate-500"><div className={state.lastTestSuccess ? 'font-bold text-emerald-600' : 'font-bold text-red-600'}>{state.lastTestSuccess ? 'Last test passed' : 'Last test failed'}</div><div>{new Date(state.lastTestAt).toLocaleString()}</div></div>}</div><div className="mt-5">{children}</div><div className="mt-4 rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-wider text-slate-500">Webhook URL</div><div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700">{webhookUrl}</code><button onClick={() => void copy()} className="rounded-lg border border-slate-200 bg-white p-2 text-slate-600" title="Copy webhook URL"><Copy className="h-4 w-4" /></button></div></div></section>;
}
...
function ProviderActions({ enabled, setEnabled, busy, provider, onSave, onTest, canTest }: { enabled: boolean; setEnabled: (value: boolean) => void; busy: string; provider: PaymentProviderId; onSave: (provider: PaymentProviderId) => Promise<void>; onTest: (provider: PaymentProviderId) => Promise<void>; canTest: boolean }) { return <div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><label className="inline-flex items-center gap-2 text-xs font-black"><input type="checkbox" checked={enabled} onChange={e => setEnabled(e.target.checked)} className="h-4 w-4" />Enable provider for client checkout</label><div className="flex gap-2"><button disabled={!!busy || !canTest} onClick={() => void onTest(provider)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2.5 text-xs font-black disabled:opacity-40">{busy === `test-${provider}` ? <Loader2 className="h-4 w-4 animate-spin" /> : <RefreshCw className="h-4 w-4" />}Test Connection</button><button disabled={!!busy} onClick={() => void onSave(provider)} className="inline-flex items-center gap-2 rounded-xl bg-[#000080] px-4 py-2.5 text-xs font-black text-white disabled:opacity-50">{busy === provider ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}Save Provider</button></div></div>; }
