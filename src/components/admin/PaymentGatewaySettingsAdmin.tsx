...
export default function PaymentGatewaySettingsAdmin() {
  ...
  const onlineCheckoutReady = [status?.razorpay, status?.paypal].some(provider => provider?.productionReady);
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    ...
    <main className="mx-auto max-w-6xl space-y-6 p-4 sm:p-8">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <StatusCard title="Online checkout" text={onlineCheckoutReady ? 'At least one verified provider is available to clients.' : 'No provider is ready. Payment links remain available for instructions and Admin-verified offline receipts.'} ok={onlineCheckoutReady} />
      </section>
      {!onlineCheckoutReady && <section className="rounded-3xl border border-amber-200 bg-amber-50 p-5 text-sm text-amber-900"><div className="flex gap-3"><XCircle className="mt-0.5 h-5 w-5 shrink-0" /><div><div className="font-black">Online collection is not live</div><p className="mt-1 leading-6">Configure and test Razorpay or PayPal, then enable it. Until that is complete, customers see a clear unavailable message and Finance can still verify an offline receipt from Payments.</p></div></div></section>}
...
function ProviderCard({ title, icon, state, webhookUrl, configuredText, webhookText, children }: { title: string; icon: React.ReactNode; state?: GatewayProviderStatus; webhookUrl: string; configuredText: string; webhookText: string; children: React.ReactNode }) {
  ...
  return <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6"><div className="flex flex-col gap-4 border-b border-slate-100 pb-5 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-3"><div className="rounded-2xl bg-slate-100 p-3 text-[#000080]">{icon}</div><div><h2 className="font-black">{title}</h2><div className="mt-1 flex flex-wrap gap-2"><Badge ok={!!state?.configured}>{configuredText}: {state?.configured ? 'configured' : 'not configured'}</Badge><Badge ok={!!state?.webhookConfigured}>{webhookText}: {state?.webhookConfigured ? 'configured' : 'not configured'}</Badge><Badge ok={!!state?.enabled}>{state?.enabled ? 'Enabled' : 'Disabled'}</Badge><Badge ok={!!state?.productionReady}>{state?.productionReady ? 'Production ready' : 'Not available to customers'}</Badge></div></div></div>{state?.lastTestAt && <div className="text-right text-[10px] text-slate-500"><div className={state.lastTestSuccess ? 'font-bold text-emerald-600' : 'font-bold text-red-600'}>{state.lastTestSuccess ? 'Last test passed' : 'Last test failed'}</div><div>{new Date(state.lastTestAt).toLocaleString()}</div></div>}</div>{state?.enabled && !state?.productionReady && <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-semibold text-amber-900">This provider is saved but blocked from customer checkout until Live mode passes a fresh server-side connection test.</div>}<div className="mt-5">{children}</div><div className="mt-4 rounded-2xl bg-slate-50 p-4"><div className="text-[10px] font-black uppercase tracking-wider text-slate-500">Webhook URL</div><div className="mt-2 flex items-center gap-2"><code className="min-w-0 flex-1 overflow-hidden text-ellipsis whitespace-nowrap text-xs text-slate-700">{webhookUrl}</code><button onClick={() => void copy()} className="rounded-lg border border-slate-200 bg-white p-2 text-slate-600" title="Copy webhook URL"><Copy className="h-4 w-4" /></button></div></div></section>;
}
