...
function ReviewDrawer({ review, setReview, snapshot, snapshotLoading, saving, onClose, onSave }: { review: SalesPerformanceReview; setReview: (review: SalesPerformanceReview | null) => void; snapshot: SalesPerformanceSnapshot | null; snapshotLoading: boolean; saving: boolean; onClose: () => void; onSave: (review: SalesPerformanceReview) => Promise<void> }) {
  ...
  return <div data-testid="part15-review-drawer" className="fixed inset-0 z-[130] flex justify-end bg-slate-950/40" onMouseDown={onClose}><div className="h-full w-full max-w-3xl overflow-y-auto bg-white p-6 shadow-2xl sm:p-8" onMouseDown={e => e.stopPropagation()}>
...
function QualityMetricCard({ metric, label }: { metric: SalesPerformanceQualityMetric; label: string }) {
...
function QualityEvidencePanel({ snapshot, compact = false }: { snapshot: SalesPerformanceSnapshot; compact?: boolean }) {
  ...
  return <section data-testid="part15-quality-evidence" className={compact ? '' : 'rounded-3xl border border-slate-200 bg-white p-6 shadow-sm'}>
    <div>
    ...
    <div className="mt-5 space-y-6">
      {QUALITY_METRIC_SECTIONS.map(section => <div key={section.title}>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">{section.keys.map(key => evidence[key] ? <QualityMetricCard key={key} metric={evidence[key]} label={QUALITY_METRIC_LABELS[key] || evidence[key].label} /> : null)}</div>
      </div>)}
    </div>
