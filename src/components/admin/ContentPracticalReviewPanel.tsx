...
export default function ContentPracticalReviewPanel({ traineeUserId, onChanged }: Props) {
  ...
  const review=async(status:'Passed'|'Retry Required')=>{
    ...
    try{
      if(status==='Passed'&&score<passScore)throw new Error(`A score of at least ${passScore}% is required to activate a Content Writer.`);
      ...
      setNotice(status==='Passed'?'Practical passed. The protected workflow activated the Content Writer.':'Retry required. The candidate returned to Content Academy with your feedback.');
      await load();
      await onChanged?.();
