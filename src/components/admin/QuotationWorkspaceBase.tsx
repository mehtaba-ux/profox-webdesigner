...
function catalogTimelineSnapshot(product: any) {
  ...
  const unit = product.deliveryDurationUnit || 'business_days';
  const impact = product.timelineImpact || 'assessment_required';
  const note = product.deliveryDurationNote || '';
  const configured = impact === 'assessment_required'
    || (product.productType === 'care_plan' && impact === 'parallel' && min == null && max == null)
...
function productDuration(product: any) {
  const snapshot = catalogTimelineSnapshot(product);
  if (product.productType === 'care_plan' && snapshot.impact === 'parallel' && snapshot.min == null && snapshot.max == null) return 'Ongoing · no delivery extension';
  if (snapshot.impact === 'assessment_required') return 'Timeline requires assessment';
  if (snapshot.status === 'missing' || snapshot.min == null || snapshot.max == null) return 'Timeline required';
  const unit = snapshot.unit === 'business_days' ? 'business days' : String(snapshot.unit).replaceAll('_', ' ');
  const range = snapshot.min === snapshot.max ? `${snapshot.min} ${unit}` : `${snapshot.min}–${snapshot.max} ${unit}`;
  if (snapshot.impact === 'additive') return `Adds approximately ${range}`;
  if (snapshot.impact === 'parallel') return `Runs in parallel · ${range}`;
  return range;
}
...
const blankLine = (type: 'custom' | 'section' | 'note', order: number): CpqLine => ({
  ...
  configurationSnapshot: type === 'custom' ? { timelineSource: 'seller_estimate', timelineStatus: 'missing' } : {},
  durationMinSnapshot: null,
  durationMaxSnapshot: null,
  durationUnitSnapshot: 'business_days',
  timelineImpactSnapshot: type === 'custom' ? 'assessment_required' : 'parallel',
  durationNoteSnapshot: ''
});
...
function lineGross(line: CpqLine) {
  return Math.max(0, Number(line.quantity || 1) * Number(line.unitPrice || 0));
}
...
function catalogLine(product: any, order: number, optional = false): CpqLine {
  ...
  return {
    ...
    configurationSnapshot: {
      ...
      catalogTimeline: timeline
    },
    durationMinSnapshot: timeline.min,
    durationMaxSnapshot: timeline.max,
    durationUnitSnapshot: timeline.unit,
    timelineImpactSnapshot: timeline.impact,
    durationNoteSnapshot: timeline.note,
...
export default function QuotationWorkspace() {
  ...
  const [draft, setDraft] = useState<CpqDraft>({
    customerName: '',
    currency: 'USD',
    status: 'Draft',
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) setError(err?.message || 'Failed to load quotation workspace.');
      } finally {
  ...
  const updateDraft = (updates: Partial<CpqDraft>) => {
    setDraft(prev => ({ ...prev, ...updates }));
    markDirty();
  };

  const setLineState = (next: CpqLine[]) => {
    setLines(next.map((line, index) => ({ ...line, sortOrder: index * 10 })));
    markDirty();
  };
  ...
  const committedTimelineLines = useMemo(
    () => lines.filter(line => ['product', 'custom'].includes(line.lineType) && !line.optionalForClient),
    [lines]
  );
  ...
  const localTotals = useMemo(() => {
    ...
    const quoteDiscount = draft.quoteDiscountType === 'percent'
      ? net * Math.min(100, Math.max(0, Number(draft.quoteDiscountValue || 0))) / 100
      : draft.quoteDiscountType === 'fixed'
        ? Math.min(net, Math.max(0, Number(draft.quoteDiscountValue || 0)))
        : 0;
  ...
  const onOpportunityChange = (id: string) => {
    ...
    if (!opportunity) {
      updateDraft({ opportunityId: null });
      return;
    }
    updateDraft({
      opportunityId: opportunity.id,
      customerName: opportunity.companyName || opportunity.name,
      contactName: opportunity.contactName || '',
      email: opportunity.email || '',
      phone: opportunity.phone || '',
      country: opportunity.country || '',
      currency: opportunity.currency || 'USD',
      salespersonId: opportunity.salespersonId || user?.id
    });
  };
  ...
  const applyTemplate = (key: string) => {
    ...
    if (!template) {
      updateDraft({ quotationTemplateKey: key });
      return;
    }
    updateDraft({
      quotationTemplateKey: key,
      proposalTitle: template.proposalTitle || draft.proposalTitle,
      coverMessage: template.coverMessage ?? draft.coverMessage,
      termsAndConditions: template.defaultTerms ?? draft.termsAndConditions
    });
  ...
  const approve = async () => {
    if (!currentId) return;
    const result = await quotationCpqService.approve(currentId);
    if (result.error) {
  ...
  const duplicate = async () => {
    if (!currentId) return;
    const result = await quotationCpqService.duplicate(currentId);
    if (result.error) {
      setError(result.error.message);
      return;
    }
    navigate(`/admin/quotations/${String(result.data)}`);
  };

  const revision = async () => {
    if (!currentId) return;
    const result = await quotationCpqService.createRevision(currentId);
    if (result.error) {
      setError(result.error.message);
      return;
    }
  ...
  const send = async () => {
    if (!currentId) return;
    const cc = sendCc.split(',').map(v => v.trim()).filter(Boolean);
    const result = await quotationCpqService.send(currentId, draft.email || '', cc, sendSubject, sendMessage);
    if (result.error) {
      setError(result.error.message);
      return;
    }
    ...
    <div className="sticky top-0 z-40 border-b border-slate-200 bg-white/95 backdrop-blur-xl">
      <div className="mx-auto flex max-w-[1800px] flex-wrap items-center justify-between gap-3 px-4 py-3 lg:px-7">
        <div className="flex min-w-0 items-center gap-3">
          <button onClick={() => navigate('/admin/app/sales?tab=quotations')} className="rounded-xl border border-slate-200 p-2 hover:bg-slate-50"><ArrowLeft className="h-4 w-4" /></button>
          <div className="min-w-0">
            <div className="flex items-center gap-2">
              <h1 className="truncate text-base font-black">{draft.quotationNumber || 'New Quotation'}</h1>
              {Number(draft.revisionNumber || 1) > 1 && <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-bold">Revision {draft.revisionNumber}</span>}
              <span className="rounded-full bg-[#000080]/10 px-2 py-0.5 text-[10px] font-extrabold text-[#000080]">{draft.status}</span>
            </div>
            <p className="mt-0.5 flex items-center gap-1 text-[11px] text-slate-500">
              {saveState === 'saving' ? <><Loader2 className="h-3 w-3 animate-spin" />Saving securely…</> : saveState === 'unsaved' ? <><Info className="h-3 w-3" />Unsaved changes</> : <><Check className="h-3 w-3 text-emerald-600" />Saved</>}
            </p>
          </div>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          {currentId && <button onClick={duplicate} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold"><Copy className="mr-1.5 inline h-3.5 w-3.5" />Duplicate</button>}
          {currentId && ['Sent', 'Rejected', 'Expired'].includes(draft.status) && !draft.supersededById && <button onClick={revision} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold"><FilePlus2 className="mr-1.5 inline h-3.5 w-3.5" />Create Revision</button>}
          <button onClick={openPreview} disabled={saving} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold"><Printer className="mr-1.5 inline h-3.5 w-3.5" />Preview</button>
          {isAdmin && <button onClick={() => navigate('/admin/quotation-settings')} className="rounded-xl border border-slate-200 p-2" title="Quotation settings"><Settings2 className="h-4 w-4" /></button>}
          {!locked && <button onClick={() => void save()} disabled={saving} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-bold"><Save className="mr-1.5 inline h-3.5 w-3.5" />Save Draft</button>}
          {draft.status === 'Draft' && <button onClick={requestApproval} className="rounded-xl bg-amber-500 px-3 py-2 text-xs font-extrabold text-white">Request Approval</button>}
          {isAdmin && draft.status === 'Ready for Approval' && <button onClick={approve} className="rounded-xl bg-emerald-600 px-3 py-2 text-xs font-extrabold text-white">Approve</button>}
          {draft.status === 'Approved' && <button onClick={openSend} className="rounded-xl bg-[#000080] px-4 py-2 text-xs font-extrabold text-white"><Send className="mr-1.5 inline h-3.5 w-3.5" />Send Quotation</button>}
        </div>
      </div>
    </div>
      ...
      <main className="space-y-5">
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex flex-col gap-4 xl:flex-row">
            <label className="flex-1"><span className="mb-1.5 block text-xs font-bold text-slate-600">CRM Opportunity</span><select disabled={locked} value={draft.opportunityId || ''} onChange={e => onOpportunityChange(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">Select opportunity</option>{opportunities.map(o => <option key={o.id} value={o.id}>{o.name} · {o.companyName}</option>)}</select></label>
            <label className="min-w-[230px]"><span className="mb-1.5 block text-xs font-bold text-slate-600">Quotation Template</span><select disabled={locked} value={draft.quotationTemplateKey || ''} onChange={e => applyTemplate(e.target.value)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm"><option value="">No template</option>{(settings.templates || []).map((t: any) => <option key={t.key} value={t.key}>{t.name}</option>)}</select></label>
          </div>
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            {[['Customer / Company', 'customerName'], ['Contact', 'contactName'], ['Email', 'email'], ['Phone', 'phone'], ['Country', 'country']].map(([label, key]) => <label key={key}><span className="mb-1 block text-[11px] font-bold text-slate-500">{label}</span><input disabled={locked} value={(draft as any)[key] || ''} onChange={e => updateDraft({ [key]: e.target.value } as any)} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" /></label>)}
            <label><span className="mb-1 block text-[11px] font-bold text-slate-500">Currency</span><select disabled={locked} value={draft.currency} onChange={e => updateDraft({ currency: e.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm">{['USD', 'EUR', 'GBP', 'CAD', 'AUD', 'INR'].map(c => <option key={c}>{c}</option>)}</select></label>
            <label><span className="mb-1 block text-[11px] font-bold text-slate-500">Valid Until</span><input disabled={locked} type="date" value={draft.validUntil || ''} onChange={e => updateDraft({ validUntil: e.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" /></label>
            <label><span className="mb-1 block text-[11px] font-bold text-slate-500">Proposal Title</span><input disabled={locked} value={draft.proposalTitle || ''} onChange={e => updateDraft({ proposalTitle: e.target.value })} className="w-full rounded-xl border border-slate-200 px-3 py-2 text-sm" /></label>
          </div>
        </section>
            ...
            <table className="w-full min-w-[1160px] text-left text-xs">
              ...
              <tbody>
                ...
                {lines.map((line, index) => <tr key={line.id || `${line.lineType}-${index}`} draggable={!locked} onDragStart={() => { draggedIndex.current = index; }} onDragOver={e => e.preventDefault()} onDrop={() => dropLine(index)} className={`${line.lineType === 'section' ? 'bg-slate-100' : line.lineType === 'note' ? 'bg-amber-50/60' : 'border-t border-slate-100'} align-top`}>
                  ...
                  <td className="p-3">{['product', 'custom'].includes(line.lineType) ? <input disabled={locked} type="number" min={1} value={line.quantity} onChange={e => updateLine(index, { quantity: Math.max(1, Number(e.target.value) || 1) })} className="w-full rounded-lg border border-slate-200 px-2 py-1.5" /> : '—'}</td>
                  <td className="p-3">{['product', 'custom'].includes(line.lineType) ? <input disabled={locked || Boolean(line.salesProductId && line.priceMode === 'fixed' && !isAdmin)} type="number" min={0} step="0.01" value={line.unitPrice} onChange={e => updateLine(index, { unitPrice: Number(e.target.value) || 0 })} className="w-full rounded-lg border border-slate-200 px-2 py-1.5" /> : '—'}</td>
        ...
        {activePanel === 'proposal' && <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div><h2 className="font-black">Customer Proposal Content</h2><p className="text-xs text-slate-500">Only customer-safe proposal content appears publicly. Internal notes never leave the workspace.</p></div>
          {[['Executive / Project Summary', 'executiveSummary'], ['Scope Summary', 'scopeSummary'], ['Client Responsibilities', 'clientResponsibilities'], ['Delivery Assumptions', 'deliveryAssumptions'], ['Review & Approval Process', 'reviewProcess'], ['Handover / Support', 'handoverSupport'], ['Exclusions', 'exclusions'], ['Terms & Conditions', 'termsAndConditions']].map(([label, key]) => <label key={key} className="block"><span className="mb-1.5 block text-xs font-bold text-slate-600">{label}</span><textarea disabled={locked} rows={key === 'executiveSummary' ? 4 : 3} value={(draft as any)[key] || ''} onChange={e => updateDraft({ [key]: e.target.value } as any)} className="w-full rounded-xl border border-slate-200 px-3 py-2.5 text-sm leading-6" /></label>)}
        </section>}

        {activePanel === 'payment' && <section className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          <div><h2 className="font-black">Payment Plan & Delivery</h2><p className="text-xs text-slate-500">Overall delivery is calculated by the existing server-side resolver from the quotation line snapshots. Sales does not calculate the final project duration manually.</p></div>
          <div className="grid gap-4 md:grid-cols-2">
            <div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-extrabold uppercase text-[#000080]">Estimated Overall Delivery</p><p className="mt-2 text-lg font-black">{presentation?.durationSnapshotText || 'Save the draft to calculate the authoritative timeline.'}</p></div>
            <div className="rounded-xl border border-slate-200 p-4"><p className="text-xs font-extrabold uppercase text-[#000080]">Payment Milestones</p>{paymentSchedule.length ? <div className="mt-3 space-y-2">{paymentSchedule.map((m: any) => <div key={m.milestoneNumber} className="flex justify-between gap-3 text-sm"><span>{m.label || m.paymentType} <span className="text-slate-400">({m.percentage}%)</span></span><b>{money(Number(m.amount || 0), draft.currency)}</b></div>)}</div> : <p className="mt-2 text-sm text-slate-500">{presentation?.paymentPlan?.error || 'Save the quotation to preview its approved payment schedule.'}</p>}</div>
          </div>
          <div className="rounded-xl border border-slate-200 p-4">
            <div className="flex items-center justify-between gap-3"><div><p className="text-xs font-extrabold uppercase text-[#000080]">Delivery Breakdown</p><p className="mt-1 text-[11px] text-slate-500">Committed product timing from this quotation snapshot.</p></div><Clock3 className="h-4 w-4 text-slate-400" /></div>
            {committedTimelineLines.length ? <div className="mt-3 divide-y divide-slate-100">{committedTimelineLines.map((line, index) => <div key={line.id || `${line.productCodeSnapshot}-${index}`} className="flex flex-col gap-1 py-2.5 sm:flex-row sm:items-start sm:justify-between sm:gap-4"><div><p className="text-xs font-bold text-slate-700">{line.productNameSnapshot}</p><p className="mt-0.5 text-[10px] font-semibold text-slate-400">{timelineSource(line) === 'catalog' ? 'Catalog Timeline' : timelineSource(line) === 'admin_override' ? 'Admin Override' : 'Seller Estimate'}</p></div><p className={`text-xs font-semibold sm:text-right ${timelineStatus(line) === 'missing' ? 'text-amber-700' : 'text-slate-600'}`}>{timelineStatus(line) === 'missing' ? 'Timeline required' : lineTimelineText(line)}</p></div>)}</div> : <p className="mt-3 text-xs text-slate-400">Add committed products or services to see the delivery breakdown.</p>}
          </div>
