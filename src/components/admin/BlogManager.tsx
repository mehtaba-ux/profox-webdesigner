...
export default function BlogManager() {
  ...
  const handleEdit = (post: BlogPost) => {
    setEditingPost({ ...post });
    setView('editor');
    setSearchParams({ tab: 'blog', edit: post.id });
  };
                  ...
                  <td className="px-6 py-5 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <button
                        ...
                        className="p-2 text-slate-500 hover:text-[#000080] hover:bg-[#000066]/10 rounded-lg transition-all cursor-pointer"
                        title="Edit Post"
...
function PostEditor({ 
  ...
  return (
    <div className="space-y-6 overflow-visible">
      {/* Editor Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-6 py-4 bg-white border border-slate-200 rounded-2xl shadow-sm">
        <div className="flex items-center gap-4">
          <button 
            type="button"
            onClick={onCancel}
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900 flex items-center gap-2">
              ...
              <span className={`text-[10px] uppercase tracking-widest px-2.5 py-0.5 rounded-full font-bold ${
                post.status === 'published' ? 'bg-[#000080]/10 text-[#000080]' : 'bg-amber-500/10 text-amber-600'
              }`}>
                {post.status}
              </span>
            </h2>
        ...
        <div className="flex flex-wrap items-center gap-3">
          {post.id && (
            <a 
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="px-3.5 py-2 text-xs font-bold text-[#000080] hover:text-[#000080] hover:bg-[#000080]/10 rounded-xl transition-all flex items-center gap-1.5 border border-[#000080]/20">
          ...
          <button 
            ...
            className="px-3.5 py-2 text-xs font-bold text-slate-700 hover:text-slate-900 bg-slate-100 hover:bg-slate-200 rounded-xl transition-all flex items-center gap-1.5 cursor-pointer"
          >
            <Eye className="w-3.5 h-3.5" />
            Preview
          </button>
          <select
            ...
            className="bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs font-semibold text-slate-900 focus:outline-none focus:border-[#000080] cursor-pointer">
            <option value="draft">Draft</option>
            <option value="published">Published</option>
          </select>
          <button 
            type="button"
            onClick={onSave}
            className="bg-[#000080] hover:bg-[#000066] text-white px-6 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all shadow-md shadow-[#000080]/20 cursor-pointer">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[1fr_320px] gap-8 items-start overflow-visible">
        {/* Main Editor Area */}
        <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 space-y-8 shadow-sm overflow-visible">
          <div className="space-y-8 overflow-visible">
            ...
            <input 
              ...
              className="w-full bg-transparent border-b border-slate-200 pb-4 text-3xl sm:text-4xl font-bold text-slate-900 placeholder-slate-400 focus:outline-none focus:border-[#000080]"
            />
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm">
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-3 p-4 bg-slate-50 rounded-xl border border-slate-200">
                <div className={`w-3 h-3 rounded-full animate-pulse ${post.status === 'published' ? 'bg-[#000080]' : 'bg-amber-500'}`} />
