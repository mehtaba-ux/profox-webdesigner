      ...
      <div className="space-y-7 p-5 sm:p-7">
        ...
        <div className="grid gap-6 xl:grid-cols-2">
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5">
            ...
            <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-4"><div className="text-xs font-black text-slate-800">Response SLA & alerts</div><p className="mt-1 text-[10px] leading-4 text-slate-500">Assignment starts the response timer and creates one task in the existing CRM activity queue.</p><div className="mt-3"><FieldInput label="First response target (minutes)" type="number" value={assignment.firstResponseSlaMinutes} onChange={value => setAssignment({ ...assignment, firstResponseSlaMinutes: Math.max(5, Math.min(1440, Number(value || 30))) })} /></div><div className="mt-3 space-y-2"><Toggle checked={assignment.notifyManagersOnNewLead} onChange={notifyManagersOnNewLead => setAssignment({ ...assignment, notifyManagersOnNewLead })} label="Notify Admins and selected managers" detail="Queue an operational alert as soon as a website enquiry arrives." /><Toggle checked={assignment.notifyAssigneeOnAssignment} onChange={notifyAssigneeOnAssignment => setAssignment({ ...assignment, notifyAssigneeOnAssignment })} label="Notify the assigned salesperson" detail="Queue an alert after assignment or reassignment." /><Toggle checked={assignment.escalateOverdueToManagers} onChange={escalateOverdueToManagers => setAssignment({ ...assignment, escalateOverdueToManagers })} label="Escalate missed SLA" detail="Notify Admins and selected managers when the first response is overdue." /></div></div>

            <div className="mt-5"><div className="flex items-center gap-2 text-xs font-black text-slate-800"><UserRoundCog className="h-4 w-4" />Who can assign leads?</div><p className="mt-1 text-[10px] leading-4 text-slate-500">Admins always can. Only Sales staff who completed CRM access setup can be selected as additional managers.</p>{assignment.managerUserIds.length===0&&<div className="mt-3 rounded-xl border border-blue-200 bg-blue-50 p-3 text-[10px] font-bold leading-5 text-blue-800">All active Admins are still notified and can assign leads. No additional Sales manager is currently selected.</div>}<div className="mt-3 max-h-44 space-y-2 overflow-auto pr-1">{snapshot.staff.map(person => <label key={person.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-white px-3 py-2.5"><span className="min-w-0"><span className="block truncate text-xs font-bold text-slate-800">{person.name}</span><span className="block truncate text-[10px] text-slate-400">{person.role}{person.department ? ` · ${person.department}` : ''}</span></span><input type="checkbox" checked={assignment.managerUserIds.includes(person.id)} onChange={() => setAssignment({ ...assignment, managerUserIds: toggleValue(assignment.managerUserIds, person.id) })} className="h-4 w-4 accent-[#000080]" /></label>)}</div></div>
