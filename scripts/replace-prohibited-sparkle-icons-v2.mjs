...
const replacementByFile = new Map([
  ...
  ['src/components/PublicBookingFlow.tsx', 'Briefcase'],
  ['src/components/QualifiedContactForm.tsx', 'BadgeCheck'],
  ['src/components/Services.tsx', 'Workflow'],
  ['src/components/admin/AdminCommissionManager.tsx', 'CircleDollarSign'],
  ['src/components/admin/AdminDashboard.tsx', 'BadgeCheck'],
  ...
  ['src/components/admin/PagesManager.tsx', 'BadgeCheck'],
  ['src/components/admin/PortfolioManager.tsx', 'Images'],
  ['src/components/admin/ProductivityCommandCenter.tsx', 'Target'],
  ['src/components/admin/ProductivityCommandPalette.tsx', 'Command'],
  ['src/components/admin/ProductivityCopilot.tsx', 'Lightbulb'],
  ['src/components/admin/QuotationWorkspaceBase.tsx', 'FilePlus2'],
  ...
  ['src/pages/CustomPageView.tsx', 'BadgeCheck'],
  ['src/pages/PortfolioDetailView.tsx', 'Images'],
  ['src/pages/PricingDetailView.tsx', 'BadgeCheck'],
  ['src/pages/PublicFeedback.tsx', 'MessageSquare'],
  ['src/pages/ServiceDetailView.tsx', 'Workflow'],
  ['src/pages/TalentPartnerProgramView.tsx', 'Handshake'],
  ['src/pages/UIUXDesignerJobView.tsx', 'Palette'],
]);

const prohibitedToken = /\b(?:sparkle|sparkles|wandsparkles)\b/gi;
const prohibitedCheck = /\b(?:sparkle|sparkles|wandsparkles)\b|✨/iu;
const textExtensions = new Set(['.ts', '.tsx', '.js', '.jsx', '.html', '.svg', '.css']);

const iconKey = (name) => name.toLowerCase();

function replaceToken(match, replacement) {
  if (match === match.toLowerCase()) return iconKey(replacement);
  return replacement;
}

function normalizeLucideImports(source) {
  return source.replace(/import\s*\{([^;]*?)\}\s*from\s*['"]lucide-react['"];?/g, (_full, body) => {
...
for (const [file, replacement] of replacementByFile) {
  ...
  let after = before.replace(prohibitedToken, match => replaceToken(match, replacement)).replaceAll('✨', '');
  after = normalizeLucideImports(after);
  if (after === before) throw new Error(`Mapped file contained no prohibited icon usage: ${file}`);
  if (prohibitedCheck.test(after)) throw new Error(`Icon cleanup incomplete in ${file}`);
...
if (remaining.length) throw new Error(`Unmapped prohibited icon usage remains:\n${remaining.join('\n')}`);

writeFileSync('sparkle-cleanup-manifest.json', `${JSON.stringify(manifest, null, 2)}\n`, 'utf8');
console.log(`Updated ${manifest.length} frontend files with semantic replacement icons.`);
