...
const cases: AcceptanceCase[] = [
  ...
  ['Meeting lineage is enforced', () => { assert.ok(has(migration, 'Meeting does not belong to this Sales lifecycle')); assert.ok(has(migration, 'service_meeting_crm_lead_id')); }],
  ['Product must be current and active', () => { assert.ok(has(migration, 'Sales product is not active or does not exist')); assert.ok(has(migration, 'sp.active')); }],
  ...
  ['severity is server-derived from policy', () => { assert.ok(has(migration, "v_severity := upper(COALESCE(v_policy->>'defaultSeverity','AMBER'))")); }],
  ['reviewer team is server-derived from policy', () => { assert.ok(has(migration, "v_team := COALESCE(v_policy->>'teamKey'")); }],
  ...
  ['stale re-review preserves supersession', () => { assert.ok(has(migration, 'supersedes_validation_id')); assert.ok(has(sellerPanel, 'supersedesValidationId')); }],
  ...
  ['specialist approval never sets CLIENT_CONFIRMED', () => { assert.ok(no(migration, "information_certainty='CLIENT_CONFIRMED'")); assert.ok(has(sellerPanel, 'does not change the Requirement certainty')); }],
  ...
  ['stale transition preserves historical decision', () => { assert.ok(has(migration, "UPDATE public.crm_sales_validations SET status='STALE',source_changed_at=now()")); assert.ok(no(routePatch, 'decision_summary=NULL')); }],
  ...
  ['fresh review can supersede stale review', () => { assert.ok(has(migration, "status IN ('STALE','REJECTED','CANCELLED')")); assert.ok(has(migration, 'v_supersedes')); }],
  ...
  ['catalog manager approval creates pre-proposal Commercial review option', () => { assert.ok(has(packageReviews, 'managerApprovalRequired')); assert.ok(has(packageReviews, "validationType: 'COMMERCIAL'")); }],
  ['timeline assessment flag creates Timeline review option', () => { assert.ok(has(packageReviews, 'timelineAssessmentRequired')); assert.ok(has(packageReviews, "validationType: 'TIMELINE'")); }],
  ...
  ['Package Fit panel embeds real review state', () => { assert.ok(has(packagePanel, '<CRMPackageFitValidationReviews')); assert.ok(has(packageReviews, 'latest.status')); }],
  ['Requirements workspace embeds Sales Validation', () => { assert.ok(has(requirements, '<CRMSalesValidationPanel')); }],
  ['Requirements needing validation show canonical review state', () => { assert.ok(has(sellerPanel, 'requirementsNeedingValidation')); assert.ok(has(sellerPanel, 'latestValidationStatus')); }],
  ['Requirements show exact Needs Information', () => { assert.ok(has(sellerPanel, 'latestInformationRequested')); assert.ok(has(sellerPanel, 'Needs Information')); }],
  ...
  ['stale decisions remain visible to Seller', () => { assert.ok(has(sellerPanel, "validation.status === 'STALE'")); }],
