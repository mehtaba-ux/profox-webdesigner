...
const cases: AcceptanceCase[] = [
  ...
  ['No specialist-review workflow implemented', () => { assert.ok(no(migration, 'create table public.sales_escalations')); assert.ok(no(migration, 'create table public.sales_validations')); assert.ok(has(panel, 'review workflow is intentionally deferred')); }],
  ...
  ['No Sales-to-Delivery handoff change', () => { assert.ok(no(migration, 'sales_to_delivery')); assert.ok(no(migration, 'handoff')); }],
