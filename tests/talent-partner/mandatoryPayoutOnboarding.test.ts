...
const portal = readFileSync(resolve(here, '../../src/pages/TalentPartnerPortal.tsx'), 'utf8');
const payoutSetup = readFileSync(resolve(here, '../../src/pages/TalentPartnerPayoutSetup.tsx'), 'utf8');
...
test('payout setup no longer depends on an approved reward', () => {
  ...
  assert.doesNotMatch(payoutSetup, /requested only after you have an approved reward/i);
  assert.doesNotMatch(portal, /requested only after a reward is approved/i);
});

test('partner UI sends incomplete onboarding directly to secure payout setup', () => {
  assert.match(portal, /accessReason==='payout_setup_required'/i);
  assert.match(portal, /function PayoutOnboardingGate/i);
  assert.match(portal, /Complete payout setup/i);
  assert.match(portal, /to="\/talent-partner\/payout-setup"/i);
  assert.match(payoutSetup, /Required before you start/i);
  assert.match(payoutSetup, /Save payout method & continue/i);
});
...
test('verification is separate from work access and sensitive helpers stay internal', () => {
  ...
  assert.match(payoutSetup, /you do not need to wait for verification before starting work/i);
